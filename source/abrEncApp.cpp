...
static void sigint_handler(int) { b_ctrl_c = 1; }
...
bool PassEncoder::readPicture(x265_picture *dstPic, int view)
{
    ...
    if (m_threadActive && ipread < ipwrite) {
        ...
        x265_picture *srcPic = (m_param->numViews > 1) ? (x265_picture *)(m_parent->m_inputPicBuffer[view][readPos])
...
void PassEncoder::threadMain()
{
    ...
    while (m_threadActive) {
        ...
        pic_recon =
            (m_cliopt.recon[0] || strlen(m_param->analysisSave) || strlen(m_param->analysisLoad) || pts_queue || reconPlay || m_param->csvLogLevel)
                ? pic_out
                : NULL;
            ...
            for (int view = 0; view < m_param->numViews - !!m_param->format; view++) {
                ...
                if (pic_in[view]) {
                    if (pic_in[view]->bitDepth > m_param->internalBitDepth && m_cliopt.bDither) {
                        x265_dither_image(pic_in[view], m_cliopt.input[view]->getWidth(), m_cliopt.input[view]->getHeight(), errorBuf,
...
void Reader::threadMain()
{
    ...
    while (m_threadActive) {
        ...
        for (int view = 0; view < m_parentEnc->m_param->numViews - !!m_parentEnc->m_param->format; view++) {
            x265_picture *dest = (m_parentEnc->m_param->numViews > 1) ? m_parentEnc->m_parent->m_inputPicBuffer[view][writeIdx]
