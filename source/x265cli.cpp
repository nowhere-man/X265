...
#define H1                       \
    ...
    H0("   --[no-]dhdr10-opt             Insert tone mapping SEI only for IDR frames and when the tone mapping information changes. Default "
       "disabled\n");
#endif
    H0("   --dolby-vision-profile <float|integer> Specifies Dolby Vision profile ID. Currently, only profile 5, profile 8.1 and profile 8.2 are "
       "enabled. Specified as '5' or '50'. Default 0 (disabled).\n");
    H0("   --dolby-vision-rpu <filename> File containing Dolby Vision RPU metadata.\n"
       "                                 If given, x265's Dolby Vision metadata parser will fill the RPU field of input pictures with the metadata "
       "read from the file. Default NULL (disabled).\n");
    H0("   --nalu-file <filename>        Text file containing SEI messages in the following format : <POC><space><PREFIX><space><NAL UNIT TYPE>/<SEI "
    ...
    H1("   --rskip-edge-threshold        Threshold in terms of percentage (an integer of range [0,100]) for minimum edge density in CU's used to "
       "prune the recursion depth.\n");
    H1("                                 Applicable only to rskip mode 2. Value is preset dependent. Default: %.f\n",
       param->edgeVarThreshold * 100.0f);
    H1("   --[no-]tskip-fast             Enable fast intra transform skipping. Default %s\n", OPT(param->bEnableTSkipFast));
    H1("   --[no-]splitrd-skip           Enable skipping split RD analysis when sum of split CU rdCost larger than one split CU rdCost for Intra CU. "
       "Default %s\n",
       OPT(param->bEnableSplitRdSkip));
    H1("   --nr-intra <integer>          An integer value in range of 0 to 2000, which denotes strength of noise reduction in intra CUs. Default "
       "0\n");
    H1("   --nr-inter <integer>          An integer value in range of 0 to 2000, which denotes strength of noise reduction in inter CUs. Default "
       "0\n");
    H0("   --ctu-info <integer>          Enable receiving ctu information asynchronously and determine the reaction to the CTU information (0, 1, 2, "
       "4, 6) Default 0\n"
       ...
       "                                    Enable this option only when planning to invoke the API function x265_encoder_ctu_info to copy ctu-info "
       "asynchronously\n");
    H0("\nCoding tools:\n");
    ...
    H1("   --masking-strength <string>   Comma separated values which specify the duration and offset for the QP increment for inter-frames when "
       "scenecut-aware-qp is enabled.\n");
    H1("   --scenecut-qp-config <file>   File containing scenecut-aware-qp mode, window duration and offsets settings required for the masking. "
    ...
    H1("                                 1 - flush all the frames even when the input is not over. Slicetype decision may change with this "
       "option.\n");
    ...
    H0("   --analysis-reuse-level <1..10>      Level of analysis reuse indicates the amount of info stored/reused in save/load mode, "
       "1:least..10:most. Now deprecated. Default %d\n",
    ...
    H0("   --refine-analysis-type <string>     Reuse analysis information received through API call. Supported options are AVC and HEVC. Default "
       "disabled - %d\n",
       param->bAnalysisType);
    ...
    H0("   --refine-intra <0..4>         Enable intra-refinement for encode that uses analysis-load.\n"
       ...
       "                                    - 1 : Functionality of (0) + evaluate all intra modes at min-cu-size's depth when current depth is one "
       "smaller than min-cu-size's depth.\n"
       "                                    - 2 : Functionality of (1) + irrespective of size evaluates all angular modes when the save encode "
    ...
    H0("   --refine-inter <0..3>         Enable inter refinement for encode that uses analysis-load.\n"
       "                                    - 0 : Forces both mode and depth from the save encode.\n"
       "                                    - 1 : Functionality of (0) + evaluate all inter modes at min-cu-size's depth when current depth is one "
       "smaller than\n"
       "                                          min-cu-size's depth. When save encode decides the current block as skip(for all sizes) evaluate "
       "skip/merge.\n"
       "                                    - 2 : Functionality of (1) + irrespective of size restricts the modes evaluated when specific modes are "
    ...
    H1("   --overscan <string>           Specify whether it is appropriate for decoder to show cropped region: unknown, show or crop. Default "
       "unknown\n");
    ...
    H1("                                 smpte240m, gbr, ycgco, bt2020nc, bt2020c, smpte2085, chroma-derived-nc, chroma-derived-c, ictcp. Default "
       "unknown\n");
    H1("   --chromaloc <integer>         Specify chroma sample location (0 to 5). Default of %d\n", param->vui.chromaSampleLocTypeTopField);
    H0("   --master-display <string>     SMPTE ST 2086 master display color volume info SEI (HDR)\n");
    H0("                                    format: G(x,y)B(x,y)R(x,y)WP(x,y)L(max,min)\n");
    H0("   --max-cll <string>            Specify content light level info SEI as \"cll,fall\" (HDR).\n");
    H0("   --video-signal-type-preset <string>    Specify combinations of color primaries, transfer characteristics, color matrix, range of luma and "
       "chroma signals, and chroma sample location\n");
    H0("                                            format: <system-id>[:<color-volume>]\n");
    H0("                                            This has higher precedence than individual VUI parameters. If any individual VUI option is "
       "specified together with this,\n");
    H0("                                            which changes the values set corresponding to the system-id or color-volume, it will be "
       "discarded.\n");
    H0("                                            The color-volume can be used only with the system-id options BT2100_PQ_YCC, BT2100_PQ_ICTCP, and "
       "BT2100_PQ_RGB.\n");
    H0("                                            system-id options and their corresponding values:\n");
    H0("                                              BT601_525:       --colorprim smpte170m --transfer smpte170m --colormatrix smpte170m --range "
       "limited --chromaloc 0\n");
    H0("                                              BT601_626:       --colorprim bt470bg --transfer smpte170m --colormatrix bt470bg --range "
       "limited --chromaloc 0\n");
    H0("                                              BT709_YCC:       --colorprim bt709 --transfer bt709 --colormatrix bt709 --range limited "
       "--chromaloc 0\n");
    H0("                                              BT709_RGB:       --colorprim bt709 --transfer bt709 --colormatrix gbr --range limited\n");
    H0("                                              BT2020_YCC_NCL:  --colorprim bt2020 --transfer bt2020-10 --colormatrix bt709 --range limited "
       "--chromaloc 2\n");
    H0("                                              BT2020_RGB:      --colorprim bt2020 --transfer smpte2084 --colormatrix bt2020nc --range "
       "limited\n");
    H0("                                              BT2100_PQ_YCC:   --colorprim bt2020 --transfer smpte2084 --colormatrix bt2020nc --range "
       "limited --chromaloc 2\n");
    H0("                                              BT2100_PQ_ICTCP: --colorprim bt2020 --transfer smpte2084 --colormatrix ictcp --range limited "
       "--chromaloc 2\n");
    H0("                                              BT2100_PQ_RGB:   --colorprim bt2020 --transfer smpte2084 --colormatrix gbr --range limited\n");
    H0("                                              BT2100_HLG_YCC:  --colorprim bt2020 --transfer arib-std-b67 --colormatrix bt2020nc --range "
       "limited --chromaloc 2\n");
    H0("                                              BT2100_HLG_RGB:  --colorprim bt2020 --transfer arib-std-b67 --colormatrix gbr --range "
       "limited\n");
    ...
    H0("   --atc-sei <integer>           Emit the alternative transfer characteristics SEI message where the integer is the preferred transfer "
       "characteristic. Default disabled\n");
    H0("   --pic-struct <integer>        Set the picture structure and emit it in the picture timing SEI message. Values in the range 0..12. See "
    ...
    H0("   --[no-]opt-qp-pps             Dynamically optimize QP in PPS (instead of default 26) based on QPs in previous GOP. Default %s\n",
       OPT(param->bOptQpPPS));
    H0("   --[no-]opt-ref-list-length-pps  Dynamically set L0 and L1 ref list length in PPS (instead of default 0) based on values in the last GOP. "
       "Default %s\n",
    ...
    H0("   --svt-preset-tuner            Enable additional faster presets of SVT; This only has to be used on top of x265's ultrafast preset. "
       "Accepts values in the range of 0-2 \n");
    ...
    H0("   --[no-]frame-rc              Enable configuring Rate Control parameters(QP, CRF or Bitrate) at frame level.Default 0\n"
       "                                Enable this option only when planning to invoke the API function x265_encoder_reconfig to configure Rate "
       "Control parameters\n");
#undef OPT
#undef H0
#undef H1
...
void CLIOptions::printStatus(uint32_t frameNum)
{
    ...
    if (framesToBeEncoded) {
        ...
        snprintf(buf, sizeof(buf), "x265 [%.1f%%] %d/%d frames, %.2f fps, %.2f kb/s, eta %d:%02d:%02d",
                 100. * frameNum / (param->chunkEnd ? param->chunkEnd : param->totalFrames), frameNum,
                 (param->chunkEnd ? param->chunkEnd : param->totalFrames), fps, bitrate, eta / 3600, (eta / 60) % 60, eta % 60);
    } else {
...
bool CLIOptions::parse(int argc, char** argv)
{
    ...
    if (svtEnabled) {
        api->param_parse(param, "svt", NULL);
    }
...
#define OPT2(name1, name2) else if (!strcmp(long_options[long_options_index].name, name1) || !strcmp(long_options[long_options_index].name, name2))
            ...
            else bError |= !!api->param_parse(param, long_options[long_options_index].name, optarg);
            if (bError) {
                const char* name = long_options_index > 0 ? long_options[long_options_index].name : argv[optind - 2];
                x265_log(NULL, X265_LOG_ERROR, "invalid argument: %s = %s\n", name, optarg);
                return true;
...
#undef OPT
    ...
    if (param->bField && param->interlaceMode) {  // Field FPS
        param->fpsNum *= 2;
        // Field height
    ...
    if (param->logLevel >= X265_LOG_INFO) {
        char buf[128];
        int p = snprintf(buf, sizeof(buf), "%dx%d fps %d/%d %sp%d", param->sourceWidth, param->sourceHeight, param->fpsNum, param->fpsDenom,
    ...
    if (reconfn[0]) {
        ...
        for (int i = 0; i < param->numLayers; i++) {
            this->recon[i] = ReconFile::open(reconfn[i], param->sourceWidth, param->sourceHeight, reconFileBitDepth, param->fpsNum, param->fpsDenom,
                                             param->internalCsp, param->sourceBitDepth);
            if (this->recon[i]->isFail()) {
                x265_log(param, X265_LOG_WARNING, "unable to write reconstructed outputs file\n");
                this->recon[i]->release();
                this->recon[i] = 0;
            } else {
                general_log(param, this->recon[i]->getName(), X265_LOG_INFO, "reconstructed images %dx%d fps %d/%d %s\n", param->sourceWidth,
                            param->sourceHeight, param->fpsNum, param->fpsDenom, x265_source_csp_names[param->internalCsp]);
            }
        }
    }
#if ENABLE_LIBVMAF
...
bool CLIOptions::parseQPFile(x265_picture& pic_org)
{
    ...
    while (num < pic_org.poc) {
        ...
        if (type == 'I') {
            pic_org.sliceType = X265_TYPE_IDR;
        } else if (type == 'i') {
            pic_org.sliceType = X265_TYPE_I;
        } else if (type == 'K') {
            pic_org.sliceType = param->bOpenGOP ? X265_TYPE_I : X265_TYPE_IDR;
        } else if (type == 'P') {
            pic_org.sliceType = X265_TYPE_P;
        } else if (type == 'B') {
            pic_org.sliceType = X265_TYPE_BREF;
        } else if (type == 'b') {
            pic_org.sliceType = X265_TYPE_B;
        } else {
            ret = 0;
        }
        if (ret < 2 || qp < -1 || qp > 51) {
...
bool CLIOptions::parseZoneFile()
{
    ...
    for (int i = 0; i < param->rc.zonefileCount; i++) {
        ...
        while (fgets(line, sizeof(line), zoneFile)) {
            ...
            line[index] = '\0';
            argLine = line;
            while (isspace((unsigned char)*argLine)) {
                argLine++;
            }
...
#define OPT(STR) else if (!strcmp(name, STR))
                ...
                if (param->numViews > 1) {
                    if (0)
                        ;
                    OPT("input") { strncpy(fn[numInput++], optarg, 1024); }
                }
#undef OPT
            }
            if (bError) {
