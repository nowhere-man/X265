...
static const char *summaryCSVHeader = "Command, Date/Time, Elapsed Time, FPS, Bitrate, "
...
int x265_encoder_headers(x265_encoder *enc, x265_nal **pp_nal, uint32_t *pi_nal)
{
    if (pp_nal && enc) {
        ...
        *pp_nal = &encoder->m_nalList.m_nal[0];
        if (pi_nal) {
            *pi_nal = encoder->m_nalList.m_numNal;
        }
        ...
        if (!isReconfigureRc) {
            ...
            if (saveVPS.ptl.profileIdc[0] != encoder->m_vps.ptl.profileIdc[0] || saveVPS.ptl.levelIdc != encoder->m_vps.ptl.levelIdc ||
                ...
                x265_log(encoder->m_param, X265_LOG_WARNING,
                         "Profile/Level/Tier has changed from %d/%d/%s to %d/%d/%s.Cannot reconfigure rate-control.\n", saveVPS.ptl.profileIdc[0],
                         saveVPS.ptl.levelIdc, saveVPS.ptl.tierFlag ? "High" : "Main", encoder->m_vps.ptl.profileIdc[0], encoder->m_vps.ptl.levelIdc,
...
{
    Encoder *encoder = static_cast<Encoder *>(enc);
    if ((totalstreamduration > 0) && (picture->poc) > ((encoder->m_param->vbvEndFrameAdjust) * (totalstreamduration) *
                                                       ((double)(encoder->m_param->fpsNum / encoder->m_param->fpsDenom)))) {
...
static void svt_print_summary(x265_encoder *enc)
{
    ...
    EB_H265_ENC_CONFIGURATION *svtParam = (EB_H265_ENC_CONFIGURATION *)encoder->m_svtAppData->svtHevcParams;
    if (svtParam->frameRateNumerator && svtParam->frameRateDenominator &&
        (svtParam->frameRateNumerator != 0 && svtParam->frameRateDenominator != 0)) {
        ...
        printf("%12d\t\t%4.2f fps\t\t%10.0f\t\t%5.2f kbps\n", (int32_t)encoder->m_svtAppData->outFrameCount, (double)frameRate,
               (double)encoder->m_svtAppData->byteCount, bitrate);
    }
}
#endif
...
void x265_alloc_analysis_data(x265_param *param, x265_analysis_data *analysis)
{
    ...
    int minReuseLevel = (param->analysisSaveReuseLevel && param->analysisLoadReuseLevel)
                            ? X265_MIN(param->analysisSaveReuseLevel, param->analysisLoadReuseLevel)
                            : maxReuseLevel;
...
x265_picture *x265_picture_alloc() { return (x265_picture *)x265_malloc(sizeof(x265_picture)); }
...
void x265_picture_free(x265_picture *p) { return x265_free(p); }
...
static const x265_api libapi = {X265_MAJOR_VERSION,
                                X265_BUILD,
                                sizeof(x265_param),
                                sizeof(x265_picture),
...
const x265_api *x265_api_query(int bitDepth, int apiVersion, int *err)
{
    if (apiVersion < 51) {
        ...
        if (err) {
            *err = X265_API_QUERY_ERR_VER_REFUSED;
        }
        return NULL;
    }

    if (err) {
        *err = X265_API_QUERY_ERR_NONE;
    }
    ...
    if (bitDepth && bitDepth != X265_DEPTH) {
        ...
        if (bitDepth == 12) {
            ...
            if (err) {
                *err = X265_API_QUERY_ERR_LIB_NOT_FOUND;
            }
            return NULL;
        }
        ...
        if (api && bitDepth != api->bit_depth) {
            x265_log(NULL, X265_LOG_WARNING, "%s does not support requested bitDepth %d\n", libname, bitDepth);
            if (err) {
                *err = X265_API_QUERY_ERR_WRONG_BITDEPTH;
            }
            return NULL;
        }

        if (err) {
            *err = api ? X265_API_QUERY_ERR_NONE : e;
        }
        return api;
    }
...
void x265_csvlog_frame(const x265_param *param, const x265_picture *pic)
{
    ...
    if (param->csvLogLevel) {
        for (uint32_t depth = 0; depth <= param->maxCUDepth; depth++) {
            fprintf(param->csvfpt, "%5.2lf%%, %5.2lf%%, %5.2lf%%,", frameStats->cuStats.percentIntraDistribution[depth][0],
                    frameStats->cuStats.percentIntraDistribution[depth][1], frameStats->cuStats.percentIntraDistribution[depth][2]);
        }
    ...
    if (param->csvLogLevel >= 2) {
        fprintf(param->csvfpt, ", %.2lf, %.2lf, %.2lf, %.2lf ", frameStats->avgLumaDistortion, frameStats->avgChromaDistortion,
                frameStats->avgPsyEnergy, frameStats->avgResEnergy);
        ...
        fprintf(param->csvfpt, ", %.1lf, %.1lf, %.1lf, %.1lf, %.1lf, %.1lf, %.1lf,", frameStats->decideWaitTime, frameStats->row0WaitTime,
                frameStats->wallTime, frameStats->refWaitWallTime, frameStats->totalCTUTime, frameStats->stallTime, frameStats->totalFrameTime);
...
void x265_csvlog_encode(const x265_param *p, const x265_stats *stats, int padx, int pady, int argc, char **argv)
{
    if (p && p->csvfpt) {
        ...
        fprintf(p->csvfpt, "%.2f, %.2f, %.2f,", stats->elapsedEncodeTime, stats->encodedPictureCount / stats->elapsedEncodeTime, stats->bitrate);

        if (p->bEnablePsnr) {
            fprintf(p->csvfpt, " %.3lf, %.3lf, %.3lf, %.3lf,", stats->globalPsnrY / stats->encodedPictureCount,
                    stats->globalPsnrU / stats->encodedPictureCount, stats->globalPsnrV / stats->encodedPictureCount, stats->globalPsnr);
        } else {
            fprintf(p->csvfpt, " -, -, -, -,");
        }
        if (p->bEnableSsim) {
...
int compute_vmaf(double *vmaf_score, char *fmt, int width, int height, int bitdepth,
                 int (*read_frame)(float *ref_data, float *main_data, float *temp_data, int stride_byte, void *user_data), void *user_data,
                 char *model_path, char *log_path, char *log_fmt, int disable_clip, int disable_avx, int enable_transform, int phone_model,
                 int do_psnr, int do_ssim, int do_ms_ssim, char *pool_method, int n_thread, int n_subsample)
{
    int err = 0;
...
double x265_calculate_vmafscore(x265_param *param, x265_vmaf_data *data)
{
    ...
    compute_vmaf(&score, (char *)pix_format, data->width, data->height, param->sourceBitDepth, read_frame, data, vcd->model_path, vcd->log_path,
                 vcd->log_fmt, vcd->disable_clip, vcd->disable_avx, vcd->enable_transform, vcd->phone_model, vcd->psnr, vcd->ssim, vcd->ms_ssim,
                 vcd->pool, vcd->thread, vcd->subsample);
...
double x265_calculate_vmaf_framelevelscore(x265_param *param, x265_vmaf_framedata *vmafframedata)
{
    ...
    compute_vmaf(&score, (char *)pix_format, vmafframedata->width, vmafframedata->height, param->sourceBitDepth, read_frame, vmafframedata,
                 vcd->model_path, vcd->log_path, vcd->log_fmt, vcd->disable_clip, vcd->disable_avx, vcd->enable_transform, vcd->phone_model,
                 vcd->psnr, vcd->ssim, vcd->ms_ssim, vcd->pool, vcd->thread, vcd->subsample);
