...
{
    int32_t v = pix[0] + pix[1] + pix[2] + pix[3] + pix[4] + pix[5] + pix[6] + pix[7] + pix[8] + pix[9] + pix[10] + pix[11];
...
{
    int32_t v = pix[0] + pix[1] + pix[2] + pix[3] + pix[4] + pix[5] + pix[6] + pix[7] + pix[8] + pix[9] + pix[10] + pix[11] + pix[12] + pix[13] +
                pix[14] + pix[15];
...
{
    int32_t v = pix[0] + pix[1] + pix[2] + pix[3] + pix[4] + pix[5] + pix[6] + pix[7] + pix[8] + pix[9] + pix[10] + pix[11] + pix[12] + pix[13] +
                pix[14] + pix[15] + pix[16] + pix[17] + pix[18] + pix[19] + pix[20] + pix[21] + pix[22] + pix[23];
...
{
    int32_t v = pix[0] + pix[1] + pix[2] + pix[3] + pix[4] + pix[5] + pix[6] + pix[7] + pix[8] + pix[9] + pix[10] + pix[11] + pix[12] + pix[13] +
                pix[14] + pix[15] + pix[16] + pix[17] + pix[18] + pix[19] + pix[20] + pix[21] + pix[22] + pix[23] + pix[24] + pix[25] + pix[26] +
                pix[27] + pix[28] + pix[29] + pix[30] + pix[31];
...
void FrameFilter::ParallelFilter::processPostCu(int col) const
{
    ...
    if ((col == 0) | (col == m_frameFilter->m_numCols - 1)) {
        ...
        if (m_frameFilter->m_param->internalCsp != X265_CSP_I400) {
            primitives.extendRowBorder(reconPic->getCbAddr(m_rowAddr), strideC, reconPic->m_picWidth >> hChromaShift, realH >> vChromaShift,
                                       reconPic->m_chromaMarginX);
            primitives.extendRowBorder(reconPic->getCrAddr(m_rowAddr), strideC, reconPic->m_picWidth >> hChromaShift, realH >> vChromaShift,
                                       reconPic->m_chromaMarginX);
        }
    }
...
void FrameFilter::processPostRow(int row, int layer)
{
    ...
    if (m_param->bEnablePsnr) {
        ...
        uint64_t ssdY =
            m_frameEncoder->m_top->computeSSD(fencPic->getLumaAddr(cuAddr), reconPic->getLumaAddr(cuAddr), stride, width, height, m_param);
        ...
        if (m_param->internalCsp != X265_CSP_I400) {
            ...
            uint64_t ssdU =
                m_frameEncoder->m_top->computeSSD(fencPic->getCbAddr(cuAddr), reconPic->getCbAddr(cuAddr), stride, width, height, m_param);
            uint64_t ssdV =
                m_frameEncoder->m_top->computeSSD(fencPic->getCrAddr(cuAddr), reconPic->getCrAddr(cuAddr), stride, width, height, m_param);
...
void FrameFilter::computeMEIntegral(int row, int layer)
{
    ...
    if (m_frame->m_lowres.sliceType != X265_TYPE_B || !layer) {
        ...
        int height = lastRow ? (maxHeight + m_param->maxCUSize * m_param->interlaceMode)
                             : (((row + m_param->interlaceMode) * m_param->maxCUSize) + m_param->maxCUSize);
