...
const MV hex4[16] = {
    MV(0, -4), MV(0, 4), MV(-2, -3), MV(2, -3), MV(-4, -2), MV(4, -2), MV(-4, -1), MV(4, -1),
    MV(-4, 0), MV(4, 0), MV(-4, 1),  MV(4, 1),  MV(-4, 2),  MV(4, 2),  MV(-2, 3),  MV(2, 3),
};
const MV offsets[] = {
    MV(-1, 0), MV(0, -1), MV(-1, -1), MV(1, -1), MV(-1, 0), MV(1, 0), MV(-1, 1), MV(-1, -1),
    MV(1, -1), MV(1, 1),  MV(-1, 0),  MV(0, 1),  MV(-1, 1), MV(1, 1), MV(1, 0),  MV(0, 1),
...
inline int predictorDifference(const MV* mvc, intptr_t numCandidates)
{
    ...
    for (int i = 0; i < numCandidates - 1; i++) {
        sum += abs(mvc[i].x - mvc[i + 1].x) + abs(mvc[i].y - mvc[i + 1].y);
    }
...
void MotionEstimate::init(int csp) { fencPUYuv.create(FENC_STRIDE, csp); }

void MotionEstimate::initScales(void)
{
#define SETUP_SCALE(W, H) sizeScale[LUMA_##W##x##H] = (H * H) >> 4;
    SETUP_SCALE(4, 4);
    SETUP_SCALE(8, 8);
    SETUP_SCALE(8, 4);
    SETUP_SCALE(4, 8);
...
int MotionEstimate::hpelIterationCount(int subme) { return workload[subme].hpel_iters + workload[subme].qpel_iters / 2; }

MotionEstimate::~MotionEstimate() { fencPUYuv.destroy(); }

/* Called by lookahead, luma only, no use of PicYuv */
void MotionEstimate::setSourcePU(pixel* fencY, intptr_t stride, intptr_t offset, int pwidth, int pheight, const int method, const int searchL0,
...
void MotionEstimate::setSourcePU(const Yuv& srcFencYuv, int _ctuAddr, int cuPartIdx, int puPartIdx, int pwidth, int pheight, const int method,
                                 const int refine, bool bChroma)
{
    partEnum = partitionFromSizes(pwidth, pheight);
    X265_CHECK(LUMA_4x4 != partEnum, "4x4 inter partition detected!\n");
    sad = primitives.pu[partEnum].sad;
...
#define COST_MV_X3_DIR(m0x, m0y, m1x, m1y, m2x, m2y, costs)                                                                                   \
    {                                                                                                                                         \
        ...
        sad_x3(fenc, pix_base + (m0x) + (m0y) * stride, pix_base + (m1x) + (m1y) * stride, pix_base + (m2x) + (m2y) * stride, stride, costs); \
...
#define COST_MV_PT_DIST_X4(m0x, m0y, p0, d0, m1x, m1y, p1, d1, m2x, m2y, p2, d2, m3x, m3y, p3, d3)                                               \
    {                                                                                                                                            \
        sad_x4(fenc, fref + (m0x) + (m0y) * stride, fref + (m1x) + (m1y) * stride, fref + (m2x) + (m2y) * stride, fref + (m3x) + (m3y) * stride, \
               stride, costs);                                                                                                                   \
...
#define COST_MV_X4(m0x, m0y, m1x, m1y, m2x, m2y, m3x, m3y)                                                                    \
    {                                                                                                                         \
        ...
        sad_x4(fenc, pix_base + (m0x) + (m0y) * stride, pix_base + (m1x) + (m1y) * stride, pix_base + (m2x) + (m2y) * stride, \
               pix_base + (m3x) + (m3y) * stride, stride, costs);                                                             \
...
#define COST_MV_X3_ABS(m0x, m0y, m1x, m1y, m2x, m2y)                                                                              \
    {                                                                                                                             \
        sad_x3(fenc, fref + (m0x) + (m0y) * stride, fref + (m1x) + (m1y) * stride, fref + (m2x) + (m2y) * stride, stride, costs); \
...
void MotionEstimate::StarPatternSearch(ReferencePlanes* ref, const MV& mvmin, const MV& mvmax, MV& bmv, int& bcost, int& bPointNr, int& bDistance,
                                       int earlyExitIters, int merange, int hme)
{
...

    {
        ...
        if (top >= mvmin.y && left >= mvmin.x && right <= mvmax.x && bottom <= mvmax.y) {
            COST_MV_PT_DIST_X4(omv.x, top, 2, dist, left, omv.y, 4, dist, right, omv.y, 5, dist, omv.x, bottom, 7, dist);
        } else {
    ...
    for (int16_t dist = 2; dist <= 8; dist <<= 1) {
        ...
        if (top >= mvmin.y && left >= mvmin.x && right <= mvmax.x && bottom <= mvmax.y)  // check border
        {
            COST_MV_PT_DIST_X4(omv.x, top, 2, dist, left2, top2, 1, dist >> 1, right2, top2, 3, dist >> 1, left, omv.y, 4, dist);
            COST_MV_PT_DIST_X4(right, omv.y, 5, dist, left2, bottom2, 6, dist >> 1, right2, bottom2, 8, dist >> 1, omv.x, bottom, 7, dist);
        ...
        if (top >= mvmin.y && left >= mvmin.x && right <= mvmax.x && bottom <= mvmax.y)  // check border
        {
          ...
          0 3 2 1 * 1 2 3 0
            ...
            */
            ...
            COST_MV_PT_DIST_X4(omv.x, top, 0, dist, left, omv.y, 0, dist, right, omv.y, 0, dist, omv.x, bottom, 0, dist);
            ...
            for (int16_t index = 1; index < 4; index++) {
                ...
                COST_MV_PT_DIST_X4(posXL, posYT, 0, dist, posXR, posYT, 0, dist, posXL, posYB, 0, dist, posXR, posYB, 0, dist);
            }
        } else  // check border for each mv
        {
            if (top >= mvmin.y)  // check top
...
void MotionEstimate::refineMV(ReferencePlanes* ref, const MV& mvmin, const MV& mvmax, const MV& qmvp, MV& outQMv)
{
    ALIGN_VAR_16(int, costs[16]);
    if (ctuAddr >= 0) {
...
int MotionEstimate::motionEstimate(ReferencePlanes* ref, const MV& mvmin, const MV& mvmax, const MV& qmvp, int numCandidates, const MV* mvc,
                                   int merange, MV& outQMv, uint32_t maxSlices, bool m_vertRestriction, pixel* srcReferencePlane)
{
    ALIGN_VAR_16(int, costs[16]);
        ...
        if (bcost & 7) {
            ...
            if ((bmv.y + hex2[dir + 1].y >= mvmin.y) & (bmv.y + hex2[dir + 1].y <= mvmax.y)) {
                ...
                for (int i = (merange >> 1) - 1; i > 0 && bmv.checkRange(mvmin, mvmax); i--) {
                    COST_MV_X3_DIR(hex2[dir + 0].x, hex2[dir + 0].y, hex2[dir + 1].x, hex2[dir + 1].y, hex2[dir + 2].x, hex2[dir + 2].y, costs);
                    bcost &= ~7;
    ...
    case X265_UMH_SEARCH: {
        ...
        if (numCandidates) {
            ...
            mvd_ctx = mvd < 10 * denom ? 0 : mvd < 20 * denom ? 1 : mvd < 40 * denom ? 2 : 3;
        ...
        uint16_t i = 1;
        do {
            if (4 * i > X265_MIN4(mvmax.x - omv.x, omv.x - mvmin.x, mvmax.y - omv.y, omv.y - mvmin.y)) {
                for (int j = 0; j < 16; j++) {
                    MV mv = omv + (hex4[j] * i);
...
#define SADS(k, x0, y0, x1, y1, x2, y2, x3, y3)                                                                                               \
    sad_x4(fenc, fref_base x0* i + (y0 - 2 * k + 4) * dy, fref_base x1 * i + (y1 - 2 * k + 4) * dy, fref_base x2 * i + (y2 - 2 * k + 4) * dy, \
           fref_base x3 * i + (y3 - 2 * k + 4) * dy, stride, costs + 4 * k);                                                                  \
    fref_base += 2 * dy;
#define ADD_MVCOST(k, x, y) costs[k] += p_cost_omvx[x * 4 * i] + p_cost_omvy[y * 4 * i]
#define MIN_MV(k, dx, dy)                                        \
    if ((omv.y + (dy) >= mvmin.y) & (omv.y + (dy) <= mvmax.y)) { \
        COPY2_IF_LT(bcost, costs[k], dir, dx * 16 + (dy & 15))   \
    }
...
#undef MIN_MV
            ...
            for (tmv.y = mvmin.y; tmv.y <= mvmax.y; tmv.y += RasterDistance) {
                for (tmv.x = mvmin.x; tmv.x <= mvmax.x; tmv.x += RasterDistance) {
                    if (tmv.x + (RasterDistance * 3) <= mvmax.x) {
                        ...
                        sad_x4(fenc, pix_base, pix_base + RasterDistance, pix_base + RasterDistance * 2, pix_base + RasterDistance * 3, stride,
                               costs);
    ...
    case X265_SEA: {
        ...
        bool smallRectPartition =
            partEnum == LUMA_4x4 || partEnum == LUMA_16x12 || partEnum == LUMA_12x16 || partEnum == LUMA_16x4 || partEnum == LUMA_4x16;
        /* Check if vertical partition */
        bool verticalRect = partEnum == LUMA_32x64 || partEnum == LUMA_16x32 || partEnum == LUMA_8x16 || partEnum == LUMA_4x8;
        /* Check if horizontal partition */
        bool horizontalRect = partEnum == LUMA_64x32 || partEnum == LUMA_32x16 || partEnum == LUMA_16x8 || partEnum == LUMA_8x4;
        /* Check if assymetric vertical partition */
        bool assymetricVertical = partEnum == LUMA_12x16 || partEnum == LUMA_4x16 || partEnum == LUMA_24x32 || partEnum == LUMA_8x32 ||
                                  partEnum == LUMA_48x64 || partEnum == LUMA_16x64;
        /* Check if assymetric horizontal partition */
        bool assymetricHorizontal = partEnum == LUMA_16x12 || partEnum == LUMA_16x4 || partEnum == LUMA_32x24 || partEnum == LUMA_32x8 ||
                                    partEnum == LUMA_64x48 || partEnum == LUMA_64x16;
        ...
        primitives.pu[tempPartEnum].sad_x4(zero, fenc, fenc + deltaX, fenc + deltaY * FENC_STRIDE, fenc + deltaX + deltaY * FENC_STRIDE, FENC_STRIDE,
                                           encDC);
        ...
        if (partEnum == LUMA_64x64 || partEnum == LUMA_32x32 || partEnum == LUMA_16x16 || partEnum == LUMA_32x64 || partEnum == LUMA_16x32 ||
            partEnum == LUMA_8x16 || partEnum == LUMA_4x8 || partEnum == LUMA_12x16 || partEnum == LUMA_4x16 || partEnum == LUMA_24x32 ||
        ...
        for (tmv.y = minY; tmv.y <= maxY; tmv.y++) {
            ...
            xn = ads(encDC, sumsBase + minX + tmv.y * stride, deltaY, fpelCostMvX + minX, meScratchBuffer, meRangeWidth, bcost);

            for (i = 0; i < xn - 2; i += 3) {
                COST_MV_X3_ABS(minX + meScratchBuffer[i], tmv.y, minX + meScratchBuffer[i + 1], tmv.y, minX + meScratchBuffer[i + 2], tmv.y);
            }
    ...
    case X265_FULL_SEARCH: {
        ...
        for (tmv.y = mvmin_y; tmv.y <= mvmax_y; tmv.y++) {
            for (tmv.x = mvmin_x; tmv.x <= mvmax_x; tmv.x++) {
                if (tmv.x + 3 <= mvmax_x) {
                    pixel* pix_base = fref + tmv.y * stride + tmv.x;
                    sad_x4(fenc, pix_base, pix_base + 1, pix_base + 2, pix_base + 3, stride, costs);
                    costs[0] += mvcost(tmv << 2);
                    COPY2_IF_LT(bcost, costs[0], bmv, tmv);
                    tmv.x++;
                    costs[1] += mvcost(tmv << 2);
    ...
    if (bChromaSATD) {
        ...
        if (!(yFrac | xFrac)) {
            ...
            if (!yFrac) {
                ...
                primitives.chroma[csp].pu[partEnum].filter_vsp(immed + (halfFilterSize - 1) * blockwidthC, blockwidthC, subpelbuf, blockwidthC,
                                                               yFrac);
                cost += chromaSatd(fencPUYuv.m_buf[1], fencStrideC, subpelbuf, blockwidthC);
