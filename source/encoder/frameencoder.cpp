...
bool FrameEncoder::init(Encoder* top, int numRows, int numCols)
{
    ...
    m_numCols = numCols;
    m_reconfigure = false;
    m_filterRowDelay = ((m_param->bEnableSAO && m_param->bSaoNonDeblocked) || (!m_param->bEnableLoopFilter && m_param->bEnableSAO))
                           ? 2
                           : (m_param->bEnableSAO || m_param->bEnableLoopFilter ? 1 : 0);
...
void FrameEncoder::threadMain()
{
    ...
    while (m_threadActive) {
        ...
        if ((m_param->bAnalysisType == AVC_INFO) && !strlen(m_param->analysisSave) && !strlen(m_param->analysisLoad) &&
            !(IS_X265_TYPE_I(m_frame[0]->m_lowres.sliceType))) {
            while (((m_frame[0]->m_analysisData.interData == NULL && m_frame[0]->m_analysisData.intraData == NULL) ||
...
void FrameEncoder::compressFrame(int layer)
{
    ...
    bool isSei = ((m_frame[layer]->m_lowres.bKeyframe && m_param->bRepeatHeaders) || m_param->bEmitHRDSEI || !!m_param->interlaceMode ||
                  (m_frame[layer]->m_lowres.sliceType == X265_TYPE_IDR && m_param->bEmitIDRRecoverySEI) || m_frame[layer]->m_userSEI.numPayloads);
    ...
    if (m_param->bDynamicRefine &&
        m_top->m_startPoint <= m_frame[layer]->m_encodeOrder) {  // Avoid collecting data that will not be used by future frames.
    ...
    if (m_param->csvLogLevel >= 1) {
        ...
        m_frame[layer]->m_encData->m_frameStats.percentIntraNxN =
            (double)(m_frame[layer]->m_encData->m_frameStats.cntIntraNxN * 100) / m_frame[layer]->m_encData->m_frameStats.totalCu;

        for (uint32_t depth = 0; depth <= m_param->maxCUDepth; depth++) {
            m_frame[layer]->m_encData->m_frameStats.percentSkipCu[depth] =
                (double)(m_frame[layer]->m_encData->m_frameStats.cntSkipCu[depth] * 100) / m_frame[layer]->m_encData->m_frameStats.totalCu;
            m_frame[layer]->m_encData->m_frameStats.percentMergeCu[depth] =
                (double)(m_frame[layer]->m_encData->m_frameStats.cntMergeCu[depth] * 100) / m_frame[layer]->m_encData->m_frameStats.totalCu;
            for (int n = 0; n < INTRA_MODES; n++) {
                m_frame[layer]->m_encData->m_frameStats.percentIntraDistribution[depth][n] =
                    (double)(m_frame[layer]->m_encData->m_frameStats.cuIntraDistribution[depth][n] * 100) /
                    m_frame[layer]->m_encData->m_frameStats.totalCu;
            }
            uint64_t cuInterRectCnt = 0;  // sum of Nx2N, 2NxN counts
            cuInterRectCnt += m_frame[layer]->m_encData->m_frameStats.cuInterDistribution[depth][1] +
                              m_frame[layer]->m_encData->m_frameStats.cuInterDistribution[depth][2];
            m_frame[layer]->m_encData->m_frameStats.percentInterDistribution[depth][0] =
                (double)(m_frame[layer]->m_encData->m_frameStats.cuInterDistribution[depth][0] * 100) /
                m_frame[layer]->m_encData->m_frameStats.totalCu;
            m_frame[layer]->m_encData->m_frameStats.percentInterDistribution[depth][1] =
                (double)(cuInterRectCnt * 100) / m_frame[layer]->m_encData->m_frameStats.totalCu;
            m_frame[layer]->m_encData->m_frameStats.percentInterDistribution[depth][2] =
                (double)(m_frame[layer]->m_encData->m_frameStats.cuInterDistribution[depth][3] * 100) /
                m_frame[layer]->m_encData->m_frameStats.totalCu;
        }
    }

    if (m_param->csvLogLevel >= 2) {
        m_frame[layer]->m_encData->m_frameStats.avgLumaDistortion =
            (double)(m_frame[layer]->m_encData->m_frameStats.lumaDistortion) / m_frame[layer]->m_encData->m_frameStats.totalCtu;
        m_frame[layer]->m_encData->m_frameStats.avgChromaDistortion =
            (double)(m_frame[layer]->m_encData->m_frameStats.chromaDistortion) / m_frame[layer]->m_encData->m_frameStats.totalCtu;
        m_frame[layer]->m_encData->m_frameStats.avgPsyEnergy =
            (double)(m_frame[layer]->m_encData->m_frameStats.psyEnergy) / m_frame[layer]->m_encData->m_frameStats.totalCtu;
        m_frame[layer]->m_encData->m_frameStats.avgSsimEnergy =
            (double)(m_frame[layer]->m_encData->m_frameStats.ssimEnergy) / m_frame[layer]->m_encData->m_frameStats.totalCtu;
        m_frame[layer]->m_encData->m_frameStats.avgResEnergy =
            (double)(m_frame[layer]->m_encData->m_frameStats.resEnergy) / m_frame[layer]->m_encData->m_frameStats.totalCtu;
    }
    ...
    if (m_param->maxSlices > 1) {
        ...
        for (uint32_t sliceId = 0; sliceId < m_param->maxSlices; sliceId++) {
            ...
            uint32_t maxStreamSize =
                m_nalList.serializeSubstreams(&m_substreamSizes[prevSliceRow], (nextSliceRow - prevSliceRow), &m_outStreams[prevSliceRow]);
            ...
            m_nalList.serialize(
                slice->m_nalUnitType, m_bs, layer,
                (!!m_param->bEnableTemporalSubLayers ? m_frame[layer]->m_tempLayer + 1 : (1 + (slice->m_nalUnitType == NAL_UNIT_CODED_SLICE_TSA_N))));
        }
    } else {
        if (m_param->bOptRefListLengthPPS) {
            ScopedLock refIdxLock(m_top->m_sliceRefIdxLock);
...
void FrameEncoder::processRowEncoder(int intRow, ThreadLocalData& tld, int layer)
{
    ...
    if (m_param->bEnableWavefront) {
        ...
        if (curRow.busy) {
            ...
            x265_log(m_param, X265_LOG_WARNING, "internal error - simultaneous row access detected. Please report HW to x265-devel@videolan.org\n");
            return;
        }
        curRow.busy = true;
    }
                ...
                if (qpoffs) {
                    ...
                    for (uint32_t cuY = cuYStart; cuY < height && (cuY < m_frame[layer]->m_fencPic->m_picHeight); cuY += qgSize) {
                        for (uint32_t cuX = 0; cuX < width; cuX += qgSize) {
                            ...
                            for (uint32_t block_yy = cuY; block_yy < cuY + qgSize && block_yy < m_frame[layer]->m_fencPic->m_picHeight;
                                 block_yy += loopIncr) {
    ...
    while (curRow.completed < numCols) {
                ...
                if (m_pool && !bFirstRowInSlice) {
                    ...
                    if (rowInSlice >= 2) {
                        allowCol = X265_MIN(((col == numCols - 1) ? m_frameFilter.m_parallelFilter[row - 2].m_lastDeblocked.get()
                                                                  : m_frameFilter.m_parallelFilter[row - 2].m_lastCol.get()),
                                            (int)col);
                    }
                ...
                if (m_pool && bLastRowInSlice) {
                    ...
                    if (rowInSlice >= 2) {
                        allowCol = X265_MIN(((col == numCols - 1) ? m_frameFilter.m_parallelFilter[row - 1].m_lastDeblocked.get()
                                                                  : m_frameFilter.m_parallelFilter[row - 1].m_lastCol.get()),
                                            (int)col);
                    }
        ...
        if (m_param->rc.bStatWrite) {
            ...
            for (uint32_t depth = 0; depth <= m_param->maxCUDepth; depth++) {
                ...
                curRow.rowStats.intra8x8Cnt +=
                    (cuSize == 8) ? (int)(frameLog.cntIntra[depth] + frameLog.cntIntraNxN) : (int)(frameLog.cntIntra[depth] << shift);
        ...
        if (!layer && bIsVbv) {
            ...
            FrameData::RCStatCU& cuStat = curEncData.m_cuStat[cuAddr];
            if ((m_param->bEnableWavefront && ((cuAddr == m_sliceBaseRow[sliceId] * numCols) || !m_param->rc.bEnableConstVbv)) ||
            ...
            if (!m_param->bEnableWavefront && col == numCols - 1) {
                ...
                if (curRow.reEncode < 0) {
                    x265_log(m_param, X265_LOG_DEBUG, "POC %d row %d - encode restart required for VBV, to %.2f from %.2f\n", m_frame[layer]->m_poc,
                             row, qpBase, curEncData.m_cuStat[cuAddr].baseQp);
        ...
        if (m_param->bEnableWavefront && curRow.completed >= 2 && !bLastRowInSlice &&
            ...
            if (m_rows[row + 1].active == false && m_rows[row + 1].completed + 2 <= curRow.completed) {
        ...
        if ((m_bAllRowsStop[curRow.sliceId] && intRow > m_vbvResetTriggerRow[curRow.sliceId]) ||
            (!bFirstRowInSlice && ((curRow.completed < numCols - 1) || (m_rows[row - 1].completed < numCols)) &&
             m_rows[row - 1].completed < curRow.completed + 2)) {
            curRow.active = false;
            curRow.busy = false;
        ...
        if (ctu.m_slice->m_sliceType == I_SLICE) {
            ...
            for (uint32_t absPartIdx = 0; absPartIdx < ctu.m_numPartitions; absPartIdx += ctu.m_numPartitions >> (depth * 2)) {
                ...
                if (ctu.m_predMode[absPartIdx] == MODE_NONE) {
                    ...
                    X265_CHECK(ctu.m_log2CUSize[absPartIdx] == 3 && ctu.m_slice->m_sps->quadtreeTULog2MinSize < 3,
...
void FrameEncoder::readModel(FilmGrainCharacteristics* m_filmGrain, FILE* filmgrain)
{
    ...
    for (int i = 0; i < MAX_NUM_COMPONENT; i++) {
        if (m_filmGrain->m_compModel[i].bPresentFlag) {
            ...
            m_filmGrain->m_compModel[i].intensityValues = (FilmGrainCharacteristics::CompModelIntensityValues*)malloc(
                sizeof(FilmGrainCharacteristics::CompModelIntensityValues) * (m_filmGrain->m_compModel[i].m_filmGrainNumIntensityIntervalMinus1 + 1));
            for (int j = 0; j <= m_filmGrain->m_compModel[i].m_filmGrainNumIntensityIntervalMinus1; j++) {
                x265_fread((char*)(&m_filmGrain->m_compModel[i].intensityValues[j].intensityIntervalLowerBound), sizeof(uint8_t), 1, filmgrain,
                           errorMessage);
                x265_fread((char*)(&m_filmGrain->m_compModel[i].intensityValues[j].intensityIntervalUpperBound), sizeof(uint8_t), 1, filmgrain,
                           errorMessage);
                m_filmGrain->m_compModel[i].intensityValues[j].compModelValue =
                    (int*)malloc(sizeof(int) * (m_filmGrain->m_compModel[i].numModelValues));
