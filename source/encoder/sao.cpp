...
inline int32_t roundIBDI(int32_t num, int32_t den) { return num >= 0 ? ((num * 2 + den) / (den * 2)) : -((-num * 2 + den) / (den * 2)); }
...
inline int64_t estSaoDist(int32_t count, int32_t offset, int32_t offsetOrg) { return (count * offset - offsetOrg * 2) * offset; }
...
void SAO::calcSaoStatsCTU(int addr, int plane)
{
...

    {
        // SAO_EO_0: // dir: -
        {
            ...
            primitives.saoCuStatsE0(diff + startX, rec0 + startX, stride, endX - startX, ctuHeight - skipB + plane_offset,
        ...
        // SAO_EO_1: // dir: |
        {
            ...
            primitives.saoCuStatsE1(diff + startY * MAX_CU_SIZE, rec0 + startY * stride, stride, upBuff1, endX, endY - startY,
                                    m_offsetOrg[plane][SAO_EO_1], m_count[plane][SAO_EO_1]);
        }
        if (!m_param->bLimitSAO || ((slice->m_sliceType == P_SLICE && !cu->isSkipped(0)) || (slice->m_sliceType != B_SLICE))) {
            // SAO_EO_2: // dir: 135
            {
                if (m_param->bSaoNonDeblocked) {
                    skipB = 4;
                ...
                primitives.saoCuStatsE2(diff + startX + startY * MAX_CU_SIZE, rec0 + startX + startY * stride, stride, upBuff1, upBufft,
                                        endX - startX, endY - startY, m_offsetOrg[plane][SAO_EO_2], m_count[plane][SAO_EO_2]);
            }
            // SAO_EO_3: // dir: 45
            {
                if (m_param->bSaoNonDeblocked) {
                ...
                primitives.saoCuStatsE3(diff + startX + startY * MAX_CU_SIZE, rec0 + startX + startY * stride, stride, upBuff1 + 1, endX - startX,
                                        endY - startY, m_offsetOrg[plane][SAO_EO_3], m_count[plane][SAO_EO_3]);
            }
        }
    }
}
...
void SAO::calcSaoStatsCu_BeforeDblk(Frame* frame, int idxX, int idxY)
{
    ...
    int plane_offset = 0;
    for (int plane = 0; plane < (frame->m_param->internalCsp != X265_CSP_I400 && m_frame->m_fencPic->m_picCsp != X265_CSP_I400 ? NUM_PLANE : 1);
         plane++) {
        if (plane == 1) {
            ...
            for (int plane = 0; plane < planes; plane++) {
                ...
                if (typeIdx >= 0) {
                    ...
                    for (int classIdx = 0; classIdx < SAO_NUM_OFFSET; classIdx++) {
                        ...
                        estDist +=
                            estSaoDist(m_count[plane][typeIdx][classIdx + bandPos], mergeOffset, m_offsetOrg[plane][typeIdx][classIdx + bandPos]);
                    }
                }
...
{
#if X265_DEPTH < 10
    X265_CHECK(bits <= (INT64_MAX - 128) / lambda, "calcRdCost wrap detected dist: " X265_LL ", bits %u, lambda: " X265_LL "\n", distortion, bits,
               lambda);
#else
    X265_CHECK(bits <= (INT64_MAX - 128) / lambda, "calcRdCost wrap detected dist: " X265_LL ", bits %u, lambda: " X265_LL "\n", distortion, bits,
               lambda);
...
void saoCuStatsE2_c(const int16_t* diff, const pixel* rec, intptr_t stride, int8_t* upBuff1, int8_t* upBufft, int endX, int endY, int32_t* stats,
                    int32_t* count)
{
    X265_CHECK(endX < MAX_CU_SIZE, "endX check failure\n");
    X265_CHECK(endY < MAX_CU_SIZE, "endY check failure\n");
