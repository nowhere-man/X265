...
struct LookaheadTLD {
    ...
    void computeIntensityHistogramBinsChroma(Frame* curFrame, uint64_t* sumAverageIntensityCb, uint64_t* sumAverageIntensityCr);

    void calculateHistogram(pixel* inputSrc, uint32_t inputWidth, uint32_t inputHeight, intptr_t stride, uint8_t dsFactor, uint32_t* histogram,
                            uint64_t* sum);
