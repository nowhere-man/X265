...
#define ProfileLookaheadTime(elapsed, count) \
    ScopedElapsedTime _scope(elapsed);       \
    count++
#else
#define ProfileLookaheadTime(elapsed, count)
#endif
    ...
    for (int rowNum = 0; rowNum < height; rowNum++) {
        for (int colNum = 0; colNum < width; colNum++) {
            if ((rowNum >= 2) && (colNum >= 2) && (rowNum < height - 2) && (colNum < width - 2))  // Ignoring the border pixels of the picture
            {
                ...
                pixelValue = ((2 * src[rowOne + colOne] + 4 * src[rowOne + colTwo] + 5 * src[rowOne + colThree] + 4 * src[rowOne + colFour] +
                               2 * src[rowOne + colFive] + 4 * src[rowTwo + colOne] + 9 * src[rowTwo + colTwo] + 12 * src[rowTwo + colThree] +
                               9 * src[rowTwo + colFour] + 4 * src[rowTwo + colFive] + 5 * src[rowThree + colOne] + 12 * src[rowThree + colTwo] +
                               15 * src[rowThree + colThree] + 12 * src[rowThree + colFour] + 5 * src[rowThree + colFive] +
                               4 * src[rowFour + colOne] + 9 * src[rowFour + colTwo] + 12 * src[rowFour + colThree] + 9 * src[rowFour + colFour] +
                               4 * src[rowFour + colFive] + 2 * src[rowFive + colOne] + 4 * src[rowFive + colTwo] + 5 * src[rowFive + colThree] +
                               4 * src[rowFive + colFour] + 2 * src[rowFive + colFive]) /
                              159);
        ...
        if (param->rc.aqMode == X265_AQ_NONE || param->rc.aqStrength == 0) {
            ...
            if (param->rc.hevcAq) {
                ...
                if (param->rc.aqMode == X265_AQ_EDGE && param->recursionSkipMode == EDGE_BASED_RSKIP) {
                    pixel* src =
                        curFrame->m_edgePic + curFrame->m_fencPic->m_lumaMarginY * curFrame->m_fencPic->m_stride + curFrame->m_fencPic->m_lumaMarginX;
                    primitives.planecopy_pp_shr(src, curFrame->m_fencPic->m_stride, curFrame->m_edgeBitPic, curFrame->m_fencPic->m_stride,
                                                curFrame->m_fencPic->m_picWidth, curFrame->m_fencPic->m_picHeight, SHIFT_TO_BITPLANE);
                }
                        ...
                        for (int blockX = 0; blockX < maxCol; blockX += loopIncr) {
                            ...
                            if (param->rc.aqMode == X265_AQ_EDGE) {
                                ...
                                if (edgeDensity) {
                                    ...
                                    if ((avgAngle >= EDGE_INCLINATION - 15 && avgAngle <= EDGE_INCLINATION + 15) ||
                                        (avgAngle >= EDGE_INCLINATION + 75 && avgAngle <= EDGE_INCLINATION + 105)) {
                ...
                for (int cuX = 0; cuX < widthInCU; cuX++) {
                    ...
                    curFrame->m_lowres.invQscaleFactor8x8[cuXY] =
                        (curFrame->m_lowres.invQscaleFactor[cuX * 2 + cuY * widthInCU * 4] +
                         ...
                         curFrame->m_lowres.invQscaleFactor[cuX * 2 + cuY * widthInCU * 4 + curFrame->m_lowres.maxBlocksInRowFullRes + 1]) /
                        4;
                }
            }
        }
...
void LookaheadTLD::lowresIntraEstimate(Lowres& fenc, uint32_t qgSize)
{
    ...
    for (int cuY = 0; cuY < heightInCU; cuY++) {
        ...
        for (int cuX = 0; cuX < widthInCU; cuX++) {
            ...
            const bool bFrameScoreCU = (cuX > 0 && cuX < widthInCU - 1 && cuY > 0 && cuY < heightInCU - 1) || widthInCU <= 2 || heightInCU <= 2;
            int icostAq;
            if (qgSize == 8) {
...
uint32_t LookaheadTLD::weightCostLuma(Lowres& fenc, Lowres& ref, WeightParam& wp)
{
    ...
    if (wp.wtPresent) {
        ...
        primitives.weight_pp(ref.buffer[0], wbuffer[0], stride, widthHeight, paddedLines, scale, round << correction, denom + correction, offset);
...
void LookaheadTLD::weightsAnalyse(Lowres& fenc, Lowres& ref)
{
    ...
    if (!found || (minscale == 1 << mindenom && minoff == 0) || (float)minscore / origscore > 0.998f) {
        ...
        for (int i = 0; i < 4; i++) {
            primitives.weight_pp(ref.buffer[i], wbuffer[i], stride, widthHeight, paddedLines, scale, round << correction, denom + correction, offset);
        }
...
Lookahead::Lookahead(x265_param* param, ThreadPool* pool)
{
    ...
    if (param->gopLookahead && (param->gopLookahead > (param->lookaheadDepth - param->bframes - 2))) {
        ...
        x265_log(param, X265_LOG_WARNING,
                 "Gop-lookahead cannot be greater than (rc-lookahead - length of the mini-gop); Clipping gop-lookahead to %d\n", param->gopLookahead);
    }
#if DETAILED_CU_STATS
    ...
    if (!strlen(curFrame->m_param->analysisLoad) || !curFrame->m_param->bDisableLookahead) {
        ...
        if (curFrame->m_param->rc.vbvBufferSize && curFrame->m_param->rc.vbvMaxBitrate) {
            ...
            if (curFrame->m_param->rc.aqMode || curFrame->m_param->bAQMotion) {
                qp_offset =
                    (frames[b]->sliceType == X265_TYPE_B || !curFrame->m_param->rc.cuTree) ? frames[b]->qpAqOffset : frames[b]->qpCuTreeOffset;
            }
                    ...
                    for (lowresCol = 0; lowresCol < widthInLowresCu; lowresCol++, lowresCuIdx++) {
                        ...
                        if (qp_offset) {
                            ...
                            if (curFrame->m_param->rc.qgSize == 8) {
                                qpOffset =
                                    (qp_offset[lowresCol * 2 + lowresRow * widthInLowresCu * 4] +
                                     ...
                                     qp_offset[lowresCol * 2 + lowresRow * widthInLowresCu * 4 + curFrame->m_lowres.maxBlocksInRowFullRes + 1]) /
                                    4;
                            } else {
...
void LookaheadTLD::computePictureStatistics(Frame* curFrame)
{
    ...
    for (int blockY = 0; blockY < maxRow; blockY += 8) {
        ...
        for (int blockX = 0; blockX < maxCol; blockX += 8) {
            ...
            variance = calcVariance(src, inpStride, blockOffsetLuma, 0);
    ...
    for (int blockY = 0; blockY < maxRowChroma; blockY += 4) {
        ...
        for (int blockX = 0; blockX < maxColChroma; blockX += 4) {
            ...
            variance = calcVariance(srcCb, cStride, blockOffsetChroma, 1);
    ...
    for (int blockY = 0; blockY < maxRowChroma; blockY += 4) {
        ...
        for (int blockX = 0; blockX < maxColChroma; blockX += 4) {
            ...
            variance = calcVariance(srcCr, cStride, blockOffsetChroma, 2);
 ...
 */
void LookaheadTLD::calculateHistogram(pixel* inputSrc, uint32_t inputWidth, uint32_t inputHeight, intptr_t stride, uint8_t dsFactor,
 ...
 */
void LookaheadTLD::computeIntensityHistogramBinsChroma(Frame* curFrame, uint64_t* sumAverageIntensityCb, uint64_t* sumAverageIntensityCr)
{
    uint64_t sum;
    uint8_t dsFactor = 4;
    ...
    for (uint32_t segmentInFrameWidthIndex = 0; segmentInFrameWidthIndex < NUMBER_OF_SEGMENTS_IN_WIDTH; segmentInFrameWidthIndex++) {
        for (uint32_t segmentInFrameHeightIndex = 0; segmentInFrameHeightIndex < NUMBER_OF_SEGMENTS_IN_HEIGHT; segmentInFrameHeightIndex++) {
            ...
            uint32_t segmentWidthOffset = (segmentInFrameWidthIndex == NUMBER_OF_SEGMENTS_IN_WIDTH - 1)
                                              ? curFrame->m_lowres.widthFullRes - (NUMBER_OF_SEGMENTS_IN_WIDTH * segmentWidth)
                                              : 0;

            uint32_t segmentHeightOffset = (segmentInFrameHeightIndex == NUMBER_OF_SEGMENTS_IN_HEIGHT - 1)
                                               ? curFrame->m_lowres.heightFullRes - (NUMBER_OF_SEGMENTS_IN_HEIGHT * segmentHeight)
                                               : 0;

            // U Histogram
            calculateHistogram(curFrame->m_fencPic->m_picOrg[1] + ((segmentInFrameWidthIndex * segmentWidth) >> 1) +
                                   (((segmentInFrameHeightIndex * segmentHeight) >> 1) * curFrame->m_fencPic->m_strideC),
                               (segmentWidth + segmentWidthOffset) >> 1, (segmentHeight + segmentHeightOffset) >> 1, curFrame->m_fencPic->m_strideC,
                               dsFactor, curFrame->m_lowres.picHistogram[segmentInFrameWidthIndex][segmentInFrameHeightIndex][1], &sum);
            ...
            *sumAverageIntensityCb += sum;
            curFrame->m_lowres.averageIntensityPerSegment[segmentInFrameWidthIndex][segmentInFrameHeightIndex][1] =
                (uint8_t)((sum + (((segmentWidth + segmentWidthOffset) * (segmentHeight + segmentHeightOffset)) >> 3)) /
                          (((segmentWidth + segmentWidthOffset) * (segmentHeight + segmentHeightOffset)) >> 2));
            ...
            // V Histogram
            calculateHistogram(curFrame->m_fencPic->m_picOrg[2] + ((segmentInFrameWidthIndex * segmentWidth) >> 1) +
                                   (((segmentInFrameHeightIndex * segmentHeight) >> 1) * curFrame->m_fencPic->m_strideC),
                               (segmentWidth + segmentWidthOffset) >> 1, (segmentHeight + segmentHeightOffset) >> 1, curFrame->m_fencPic->m_strideC,
                               dsFactor, curFrame->m_lowres.picHistogram[segmentInFrameWidthIndex][segmentInFrameHeightIndex][2], &sum);
            ...
            *sumAverageIntensityCr += sum;
            curFrame->m_lowres.averageIntensityPerSegment[segmentInFrameWidthIndex][segmentInFrameHeightIndex][2] =
                (uint8_t)((sum + (((segmentWidth + segmentWidthOffset) * (segmentHeight + segmentHeightOffset)) >> 3)) /
                          (((segmentWidth + segmentHeightOffset) * (segmentHeight + segmentHeightOffset)) >> 2));
 ...
 */
void LookaheadTLD::computeIntensityHistogramBinsLuma(Frame* curFrame, uint64_t* sumAvgIntensityTotalSegmentsLuma)
{
    uint64_t sum;
    ...
    for (uint32_t segmentInFrameWidthIndex = 0; segmentInFrameWidthIndex < NUMBER_OF_SEGMENTS_IN_WIDTH; segmentInFrameWidthIndex++) {
        for (uint32_t segmentInFrameHeightIndex = 0; segmentInFrameHeightIndex < NUMBER_OF_SEGMENTS_IN_HEIGHT; segmentInFrameHeightIndex++) {
            ...
            uint32_t segmentWidthOffset = (segmentInFrameWidthIndex == NUMBER_OF_SEGMENTS_IN_WIDTH - 1)
                                              ? curFrame->m_lowres.quarterSampleLowResWidth - (NUMBER_OF_SEGMENTS_IN_WIDTH * segmentWidth)
                                              : 0;

            uint32_t segmentHeightOffset = (segmentInFrameHeightIndex == NUMBER_OF_SEGMENTS_IN_HEIGHT - 1)
                                               ? curFrame->m_lowres.quarterSampleLowResHeight - (NUMBER_OF_SEGMENTS_IN_HEIGHT * segmentHeight)
                                               : 0;

            // Y Histogram
            calculateHistogram(curFrame->m_lowres.quarterSampleLowResBuffer +
                                   (curFrame->m_lowres.quarterSampleLowResOriginX + segmentInFrameWidthIndex * segmentWidth) +
                                   ((curFrame->m_lowres.quarterSampleLowResOriginY + segmentInFrameHeightIndex * segmentHeight) *
                                    curFrame->m_lowres.quarterSampleLowResStrideY),
                               segmentWidth + segmentWidthOffset, segmentHeight + segmentHeightOffset, curFrame->m_lowres.quarterSampleLowResStrideY,
                               1, curFrame->m_lowres.picHistogram[segmentInFrameWidthIndex][segmentInFrameHeightIndex][0], &sum);

            curFrame->m_lowres.averageIntensityPerSegment[segmentInFrameWidthIndex][segmentInFrameHeightIndex][0] =
                (uint8_t)((sum + (((segmentWidth + segmentWidthOffset) * (segmentWidth + segmentHeightOffset)) >> 1)) /
                          ((segmentWidth + segmentWidthOffset) * (segmentHeight + segmentHeightOffset)));
...
void LookaheadTLD::collectPictureStatistics(Frame* curFrame)
{
    ...
    computeIntensityHistogramBinsLuma(curFrame, &sumAverageIntensity);

    // Histogram bins for Chroma
    computeIntensityHistogramBinsChroma(curFrame, &sumAverageIntensityCb, &sumAverageIntensityCr);

    curFrame->m_lowres.averageIntensity[0] =
        (uint8_t)((sumAverageIntensity + ((curFrame->m_lowres.widthFullRes * curFrame->m_lowres.heightFullRes) >> 1)) /
                  (curFrame->m_lowres.widthFullRes * curFrame->m_lowres.heightFullRes));
    curFrame->m_lowres.averageIntensity[1] =
        (uint8_t)((sumAverageIntensityCb + ((curFrame->m_lowres.widthFullRes * curFrame->m_lowres.heightFullRes) >> 3)) /
                  ((curFrame->m_lowres.widthFullRes * curFrame->m_lowres.heightFullRes) >> 2));
    curFrame->m_lowres.averageIntensity[2] =
        (uint8_t)((sumAverageIntensityCr + ((curFrame->m_lowres.widthFullRes * curFrame->m_lowres.heightFullRes) >> 3)) /
                  ((curFrame->m_lowres.widthFullRes * curFrame->m_lowres.heightFullRes) >> 2));
...
void CostEstimateGroup::estimatelowresmotion(MotionEstimatorTLD& m_metld, Frame* curframe, int refId)
{
    ...
    m_metld.motionEstimationLuma(m_metld, ref->mvs0, ref->mvsStride0, curframe->m_lowres.lowerResPlane[0], (int)(curframe->m_lowres.lumaStride / 2),
                                 (curframe->m_lowres.lines / 2), (curframe->m_lowres.width / 2), ref->lowerRes, 16,
                                 curframe->m_param->searchRangeForLayer2);
    m_metld.motionEstimationLuma(m_metld, ref->mvs1, ref->mvsStride1, curframe->m_lowres.lowresPlane[0], (int)(curframe->m_lowres.lumaStride),
                                 (curframe->m_lowres.lines), (curframe->m_lowres.width), ref->lowres, 16, curframe->m_param->searchRangeForLayer1,
                                 ref->mvs0, ref->mvsStride0, 2);
    m_metld.motionEstimationLuma(m_metld, ref->mvs2, ref->mvsStride2, curframe->m_fencPic->m_picOrg[0], (int)curframe->m_fencPic->m_stride,
                                 curframe->m_fencPic->m_picHeight, curframe->m_fencPic->m_picWidth, ref->picBuffer->m_picOrg[0], 16,
                                 curframe->m_param->searchRangeForLayer0, ref->mvs1, ref->mvsStride1, 2);
    m_metld.motionEstimationLumaDoubleRes(m_metld, ref->mvs, ref->mvsStride, curframe->m_fencPic, ref->picBuffer, 8, ref->mvs2, ref->mvsStride2, 1,
                                          ref->error);
...
bool Lookahead::generatemcstf(Frame* frameEnc, PicList refPic, int poclast)
{
    ...
    for (int iterPOC = (frameEnc->m_poc - frameEnc->m_mcstf->m_range); iterPOC <= (frameEnc->m_poc + frameEnc->m_mcstf->m_range); iterPOC++) {
        bool isFound = false;
                ...
                for (int j = 0; j < (2 * frameEnc->m_mcstf->m_range); j++) {
                    ...
                    if (iterPOC >= poclast) {
                        ...
                        while (mcstf->m_numRef) {
                            memset(frameEnc->m_mcstfRefList[mcstf->m_numRef].mvs0, 0,
                                   sizeof(MV) * ((mcstf->m_sourceWidth / 16) * (mcstf->m_sourceHeight / 16)));
                            memset(frameEnc->m_mcstfRefList[mcstf->m_numRef].mvs1, 0,
                                   sizeof(MV) * ((mcstf->m_sourceWidth / 16) * (mcstf->m_sourceHeight / 16)));
                            memset(frameEnc->m_mcstfRefList[mcstf->m_numRef].mvs2, 0,
                                   sizeof(MV) * ((mcstf->m_sourceWidth / 16) * (mcstf->m_sourceHeight / 16)));
                            memset(frameEnc->m_mcstfRefList[mcstf->m_numRef].mvs, 0,
                                   sizeof(MV) * ((mcstf->m_sourceWidth / 4) * (mcstf->m_sourceHeight / 4)));
                            memset(frameEnc->m_mcstfRefList[mcstf->m_numRef].noise, 0,
                                   sizeof(int) * ((mcstf->m_sourceWidth / 4) * (mcstf->m_sourceHeight / 4)));
                            memset(frameEnc->m_mcstfRefList[mcstf->m_numRef].error, 0,
                                   sizeof(int) * ((mcstf->m_sourceWidth / 4) * (mcstf->m_sourceHeight / 4)));
...
void Lookahead::slicetypeDecide()
{
    ...
    if (m_param->bEnableFades) {
        ...
        for (int k = list[0]->m_poc % length; k <= list[j - 1]->m_poc % length; k++) {
            ...
            if ((k > 0 && m_frameVariance[k] >= m_frameVariance[k - 1]) || (k == 0 && m_frameVariance[k] >= m_frameVariance[length - 1])) {
                m_isFadeIn = true;
    ...
    if (m_lastNonB && ((m_param->bFrameAdaptive && m_param->bframes) || m_param->rc.cuTree || m_param->scenecutThreshold ||
                       m_param->bHistBasedSceneCut || (m_param->lookaheadDepth && m_param->rc.vbvBufferSize))) {
    ...
    if (!strlen(m_param->analysisLoad) || m_param->bAnalysisType == HEVC_INFO) {
        ...
        for (bframes = 0, brefs = 0;; bframes++) {
            ...
            else if (frm.sliceType == X265_TYPE_BREF && m_param->bBPyramid && brefs && m_param->maxNumReferences <= (brefs + 3)) {
                frm.sliceType = X265_TYPE_B;
            ...
            while ((gopId >= 0) && (leftOver > 3)) {
                if (leftOver < gopLen) {
                    ...
                    list[newbFrames]->m_lowres.sliceType =
                        IS_X265_TYPE_I(list[newbFrames]->m_lowres.sliceType) ? list[newbFrames]->m_lowres.sliceType : X265_TYPE_P;
                    if (newbFrames) {
    ...
    if (framecnt) {
        ...
        if (m_param->bResetZoneConfig) {
            for (int i = 0; i < m_param->rc.zonefileCount; i++) {
                ...
                curZoneStart += curZoneStart ? m_param->rc.zones[i].zoneParam->radl : 0;
                nextZoneStart += (i + 1 < m_param->rc.zonefileCount) ? m_param->rc.zones[i + 1].startFrame + m_param->rc.zones[i + 1].zoneParam->radl
                                                                     : m_param->totalFrames;
    ...
    if (bRealScenecut && m_param->bframes) {
        ...
        if (noScenecuts) {
            ...
            for (int i = p1; i <= maxp1; i++) {
                ...
                if (fabs((double)(curCost - avgSatdCost)) > 0.1 * avgSatdCost || fabs((double)(curCost - prevCost)) > 0.1 * prevCost) {
                    fluctuate = true;
...
bool Lookahead::scenecutInternal(Lowres** frames, int p0, int p1, bool bRealScenecut)
{
    ...
    if (bRealScenecut) {
        ...
        if (gopSize <= m_param->keyframeMin / 4 || m_param->bIntraRefresh) {
            ...
            bias = threshMin + (threshMax - threshMin) * (gopSize - m_param->keyframeMin) / (m_param->keyframeMax - m_param->keyframeMin);
        }
    }
...
bool Lookahead::detectHistBasedSceneChange(Lowres** frames, int p0, int p1, int p2)
{
    ...
    for (uint32_t segmentInFrameWidthIndex = 0; segmentInFrameWidthIndex < NUMBER_OF_SEGMENTS_IN_WIDTH; segmentInFrameWidthIndex++) {
        for (uint32_t segmentInFrameHeightIndex = 0; segmentInFrameHeightIndex < NUMBER_OF_SEGMENTS_IN_HEIGHT; segmentInFrameHeightIndex++) {
            ...
            uint32_t segmentWidthOffset = (segmentInFrameWidthIndex == NUMBER_OF_SEGMENTS_IN_WIDTH - 1)
                                              ? frames[1]->widthFullRes - (NUMBER_OF_SEGMENTS_IN_WIDTH * segmentWidth)
                                              : 0;

            uint32_t segmentHeightOffset = (segmentInFrameHeightIndex == NUMBER_OF_SEGMENTS_IN_HEIGHT - 1)
                                               ? frames[1]->heightFullRes - (NUMBER_OF_SEGMENTS_IN_HEIGHT * segmentHeight)
                                               : 0;
            ...
            uint32_t segmentThreshHold =
                (((X265_ABS((int64_t)currentFrame->picAvgVariance - (int64_t)previousFrame->picAvgVariance)) > PICTURE_DIFF_VARIANCE_TH) &&
                 (currentFrame->picAvgVariance > PICTURE_VARIANCE_TH || previousFrame->picAvgVariance > PICTURE_VARIANCE_TH))
                    ? HIGH_VAR_SCENE_CHANGE_TH * NUM64x64INPIC(segmentWidth, segmentHeight)
                    : LOW_VAR_SCENE_CHANGE_TH * NUM64x64INPIC(segmentWidth, segmentHeight);
            ...
            uint32_t segmentThreshHoldCb =
                (((X265_ABS((int64_t)currentFrame->picAvgVarianceCb - (int64_t)previousFrame->picAvgVarianceCb)) > PICTURE_DIFF_VARIANCE_CHROMA_TH) &&
                 (currentFrame->picAvgVarianceCb > PICTURE_VARIANCE_CHROMA_TH || previousFrame->picAvgVarianceCb > PICTURE_VARIANCE_CHROMA_TH))
                    ? HIGH_VAR_SCENE_CHANGE_CHROMA_TH * NUM64x64INPIC(segmentWidth, segmentHeight)
                    : LOW_VAR_SCENE_CHANGE_CHROMA_TH * NUM64x64INPIC(segmentWidth, segmentHeight);
            ...
            for (uint32_t bin = 0; bin < HISTOGRAM_NUMBER_OF_BINS; ++bin) {
                accHistDiff += X265_ABS((int32_t)currentFrame->picHistogram[segmentInFrameWidthIndex][segmentInFrameHeightIndex][0][bin] -
                                        (int32_t)previousFrame->picHistogram[segmentInFrameWidthIndex][segmentInFrameHeightIndex][0][bin]);
                accHistDiffCb += X265_ABS((int32_t)currentFrame->picHistogram[segmentInFrameWidthIndex][segmentInFrameHeightIndex][1][bin] -
                                          (int32_t)previousFrame->picHistogram[segmentInFrameWidthIndex][segmentInFrameHeightIndex][1][bin]);
                accHistDiffCr += X265_ABS((int32_t)currentFrame->picHistogram[segmentInFrameWidthIndex][segmentInFrameHeightIndex][2][bin] -
            ...
            uint32_t accHistDiffError =
                X265_ABS((int32_t)accHistDiffRunningAvg[segmentInFrameWidthIndex][segmentInFrameHeightIndex] - (int32_t)accHistDiff);
            uint32_t accHistDiffErrorCb =
                X265_ABS((int32_t)accHistDiffRunningAvgCb[segmentInFrameWidthIndex][segmentInFrameHeightIndex] - (int32_t)accHistDiffCb);
            uint32_t accHistDiffErrorCr =
                X265_ABS((int32_t)accHistDiffRunningAvgCr[segmentInFrameWidthIndex][segmentInFrameHeightIndex] - (int32_t)accHistDiffCr);
            ...
            if (isAbruptChange) {
                ...
                if (absIntDiffFuturePresent >= FLASH_TH * absIntDiffFuturePast && absIntDiffPresentPast >= FLASH_TH * absIntDiffFuturePast) {
                    ...
                    x265_log(m_param, X265_LOG_DEBUG, "Fade in frame# %i , %i, %i, %i\n", currentFrame->frameNum, absIntDiffFuturePast,
                             absIntDiffFuturePresent, absIntDiffPresentPast);
                } else if (X265_ABS(absIntDiffFuturePresent - absIntDiffPresentPast) < INTENSITY_CHANGE_TH &&
                           absIntDiffFuturePresent + absIntDiffPresentPast >= absIntDiffFuturePast) {
                ...
                accHistDiffRunningAvg[segmentInFrameWidthIndex][segmentInFrameHeightIndex] =
                    (3 * accHistDiffRunningAvg[segmentInFrameWidthIndex][segmentInFrameHeightIndex] + accHistDiff) / 4;
            }
...
void Lookahead::estimateCUPropagate(Lowres** frames, double averageDuration, int p0, int p1, int b, int referenced)
{
    ...
    for (uint16_t blocky = 0; blocky < m_8x8Height; blocky++) {
        int cuIndex = blocky * strideInCU;
        if (m_param->rc.qgSize == 8) {
            primitives.propagateCost(m_scratch, propagateCost, frames[b]->intraCost + cuIndex, frames[b]->lowresCosts[b - p0][p1 - b] + cuIndex,
                ...
                for (int cuX = 0; cuX < m_8x8Width; cuX++) {
                    ...
                    if (intracost) {
                        ...
                        frame->qpCuTreeOffset[cuX * 2 + cuY * m_8x8Width * 4] =
                            frame->qpAqOffset[cuX * 2 + cuY * m_8x8Width * 4] - m_cuTreeStrength * (log2_ratio);
                        frame->qpCuTreeOffset[cuX * 2 + cuY * m_8x8Width * 4 + 1] =
                            frame->qpAqOffset[cuX * 2 + cuY * m_8x8Width * 4 + 1] - m_cuTreeStrength * (log2_ratio);
                        frame->qpCuTreeOffset[cuX * 2 + cuY * m_8x8Width * 4 + frame->maxBlocksInRowFullRes] =
                            frame->qpAqOffset[cuX * 2 + cuY * m_8x8Width * 4 + frame->maxBlocksInRowFullRes] - m_cuTreeStrength * (log2_ratio);
                        frame->qpCuTreeOffset[cuX * 2 + cuY * m_8x8Width * 4 + frame->maxBlocksInRowFullRes + 1] =
                            frame->qpAqOffset[cuX * 2 + cuY * m_8x8Width * 4 + frame->maxBlocksInRowFullRes + 1] - m_cuTreeStrength * (log2_ratio);
                    }
                }
            }
        } else {
    ...
    if (m_param->rc.hevcAq) {
        ...
        for (int cuy = m_8x8Height - 1; cuy >= 0; cuy--) {
            ...
            for (int cux = m_8x8Width - 1; cux >= 0; cux--) {
                ...
                if (m_param->rc.qgSize == 8) {
                    qp_adj = (pcQPCuTree[cux * 2 + cuy * m_8x8Width * 4] + pcQPCuTree[cux * 2 + cuy * m_8x8Width * 4 + 1] +
                              pcQPCuTree[cux * 2 + cuy * m_8x8Width * 4 + frames[b]->maxBlocksInRowFullRes] +
                              pcQPCuTree[cux * 2 + cuy * m_8x8Width * 4 + frames[b]->maxBlocksInRowFullRes + 1]) /
                             4;
                } else {
                ...
                rowSatd[cuy] += cuCost;
                if ((cuy > 0 && cuy < m_8x8Height - 1 && cux > 0 && cux < m_8x8Width - 1) || m_8x8Width <= 2 || m_8x8Height <= 2) {
                    score += cuCost;
                }
            }
        }
        ...
        for (int cuy = m_8x8Height - 1; cuy >= 0; cuy--) {
            ...
            for (int cux = m_8x8Width - 1; cux >= 0; cux--) {
                ...
                double qp_adj;
                if (m_param->rc.qgSize == 8) {
                    qp_adj = (qp_offset[cux * 2 + cuy * m_8x8Width * 4] + qp_offset[cux * 2 + cuy * m_8x8Width * 4 + 1] +
                              qp_offset[cux * 2 + cuy * m_8x8Width * 4 + frames[b]->maxBlocksInRowFullRes] +
                              qp_offset[cux * 2 + cuy * m_8x8Width * 4 + frames[b]->maxBlocksInRowFullRes + 1]) /
                             4;
                } else {
    ...
    while (m_jobAcquired < m_jobTotal) {
        ...
        if (m_batchMode) {
            ...
            if (m_lookahead.m_param->bEnableTemporalFilter && curFrame &&
                (curFrame->m_lowres.sliceType == X265_TYPE_IDR || curFrame->m_lowres.sliceType == X265_TYPE_I ||
                 curFrame->m_lowres.sliceType == X265_TYPE_P)) {
...
int64_t CostEstimateGroup::estimateFrameCost(LookaheadTLD& tld, int p0, int p1, int b, bool bIntraPenalty)
{
    ...
    if (fenc->costEst[b - p0][p1 - b] >= 0 && fenc->rowSatds[b - p0][p1 - b][0] != -1) {
        ...
        if (bDoSearch[0]) {
            fenc->lowresMvs[0][b - p0][0].x = 0x7FFE;
        }
        if (bDoSearch[1]) {
            fenc->lowresMvs[1][p1 - b][0].x = 0x7FFE;
        }
#endif
                                       ...
                                       bool hme)
{
    ...
    if ((bBidir || bDoSearch[0] || bDoSearch[1]) && hme) {
        tld.me.setSourcePU(fenc->lowerResPlane[0], fenc->lumaStride / 2, pelOffset, cuSize, cuSize, X265_HEX_SEARCH,
                           m_lookahead.m_param->hmeSearchMethod[0], m_lookahead.m_param->hmeSearchMethod[1], 1);
    } else if ((bBidir || bDoSearch[0] || bDoSearch[1]) && !hme) {
        tld.me.setSourcePU(fenc->lowresPlane[0], fenc->lumaStride, pelOffset, cuSize, cuSize, X265_HEX_SEARCH,
                           m_lookahead.m_param->hmeSearchMethod[0], m_lookahead.m_param->hmeSearchMethod[1], 1);
    }
