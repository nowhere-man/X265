...
#define MAX_PFILTER_CUS (4) /* maximum CUs for every thread */
    ...
    public:
        ...
        ParallelFilter() : m_rowHeight(0), m_row(0), m_rowAddr(0), m_frameFilter(NULL), m_encData(NULL), m_prevRow(NULL) { }
        ...
        uint32_t getCUHeight() const { return m_rowHeight; }
    };

    ParallelFilter* m_parallelFilter;

    FrameFilter() : m_param(NULL), m_frame(NULL), m_frameEncoder(NULL), m_ssimBuf(NULL), m_parallelFilter(NULL) { }

    uint32_t getCUWidth(int colNum) const { return (colNum == (int)m_numCols - 1) ? m_lastWidth : m_param->maxCUSize; }
