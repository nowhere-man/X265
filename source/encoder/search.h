...
struct MotionData {
    ...
    MotionData() { memset(this, 0, sizeof(MotionData)); }
};
...
inline int getTUBits(int idx, int numIdx) { return idx + (idx < numIdx - 1); }
...
public:
    ...
    bool predIntraBCSearch(Mode& intraBCMode, const CUGeom& cuGeom, bool bChromaMC, PartSize ePartSize, bool testOnlyPred, bool bUse1DSearchFor8x8,
                           IBC& ibc);
    void intraBlockCopyEstimate(Mode& intraBCMode, const CUGeom& cuGeom, int puIdx, MV* pred, MV& mv, uint32_t& cost, bool testOnlyPred,
    ...
    void intraPatternSearch(Mode& intraBCMode, const CUGeom& cuGeom, int puIdx, uint32_t partAddr, pixel* refY, int refStride, MV* searchRangeLT,
    ...
    bool isBlockVectorValid(int xPos, int yPos, int width, int height, CUData* pcCU, int xStartInCU, int yStartInCU, int xBv, int yBv, int ctuSize);
    ...
    int intraBCSearchMVChromaRefine(Mode& intraBCMode, const CUGeom& cuGeom, int roiWidth, int roiHeight, int cuPelX, int cuPelY,
                                    uint32_t* sadBestCand, MV* cMVCand, uint32_t partOffset, int puIdx);
    ...
    void restrictBipredMergeCand(CUData* cu, uint32_t puIdx, MVField (*mvFieldNeighbours)[2], uint8_t* interDirNeighbours,
...
protected:
    ...
    bool splitTU(Mode& mode, const CUGeom& cuGeom, uint32_t absPartIdx, uint32_t tuDepth, ShortYuv& resiYuv, Cost& splitCost,
                 const uint32_t depthRange[2], int32_t splitMore);
    void estimateResidualQT(Mode& mode, const CUGeom& cuGeom, uint32_t absPartIdx, uint32_t depth, ShortYuv& resiYuv, Cost& costs,
                            const uint32_t depthRange[2], int32_t splitMore = -1);

    // generate prediction, generate residual and recon. if bAllowSplit, find optimal RQT splits
    void codeIntraLumaQT(Mode& mode, const CUGeom& cuGeom, uint32_t tuDepth, uint32_t absPartIdx, bool bAllowSplit, Cost& costs,
    ...
    void updateModeCost(Mode& m) const
    {
        m.rdCost = m_rdCost.m_psyRd ? m_rdCost.calcPsyRdCost(m.distortion, m.totalBits, m.psyEnergy)
