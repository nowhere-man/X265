...
Mode& Analysis::compressCTU(CUData& ctu, Frame& frame, const CUGeom& cuGeom, const Entropy& initialContext)
{
    ...
    if (m_slice->m_sliceType == I_SLICE || (m_param->bEnableSCC && (m_slice->m_numRefIdx[0] == 1) && m_slice->m_refPOCList[0][0] == m_slice->m_poc)) {
        ...
        bool bCompressInterCUrd5_6 =
            (m_param->bAnalysisType == AVC_INFO && m_param->analysisLoadReuseLevel >= 7 && m_param->rdLevel >= 5 && m_param->rdLevel <= 6);
        ...
        if (m_param->bIntraRefresh && m_slice->m_sliceType == P_SLICE && ctu.m_cuPelX / m_param->maxCUSize >= frame.m_encData->m_pir.pirStartCol &&
    ...
    for (uint32_t absPartIdx = 0; absPartIdx < ctu.m_numPartitions; absPartIdx += ctu.m_numPartitions >> (depth * 2)) {
        ...
        for (uint32_t puIdx = 0; puIdx < numPU; puIdx++) {
            ...
            if (ctu.m_partSize[puabsPartIdx + absPartIdx] == SIZE_Nx2N || ctu.m_partSize[puabsPartIdx + absPartIdx] == SIZE_2NxN) {
                mode = 2;
            } else if (ctu.m_partSize[puabsPartIdx + absPartIdx] == SIZE_2NxnU || ctu.m_partSize[puabsPartIdx + absPartIdx] == SIZE_2NxnD ||
                       ctu.m_partSize[puabsPartIdx + absPartIdx] == SIZE_nLx2N || ctu.m_partSize[puabsPartIdx + absPartIdx] == SIZE_nRx2N) {
                mode = 3;
            }
...
#endif
{
    ...
    bool bAlreadyDecided =
        m_param->intraRefine != 4 && parentCTU.m_lumaIntraDir[cuGeom.absPartIdx] != (uint8_t)ALL_IDX && !(m_param->bAnalysisType == HEVC_INFO);
    ...
    if (m_param->intraRefine && m_param->intraRefine != 4) {
        split = m_param->scaleFactor && bDecidedDepth && (!mightNotSplit || ((cuGeom.log2CUSize == (uint32_t)(g_log2Size[m_param->minCUSize] + 1))));
    ...
    if (bAlreadyDecided) {
        if (bDecidedDepth && mightNotSplit) {
            ...
            bool reuseModes = !((m_param->intraRefine == 3) || (m_param->intraRefine == 2 && parentCTU.m_lumaIntraDir[cuGeom.absPartIdx] > DC_IDX));
            if (reuseModes) {
...
uint32_t Analysis::compressInterCU_dist(const CUData& parentCTU, const CUGeom& cuGeom, int32_t qp)
{
    ...
    if (mightNotSplit && depth >= minDepth) {
        ...
        int bTryIntra = (m_slice->m_sliceType != B_SLICE || m_param->bIntraInBFrames) && (!m_param->limitReferences || splitIntra) &&
                        (cuGeom.log2CUSize != MAX_LOG2_CU_SIZE);
        ...
        m_splitRefIdx[0] = splitRefs[0];
        m_splitRefIdx[1] = splitRefs[1];
        m_splitRefIdx[2] = splitRefs[2];
        m_splitRefIdx[3] = splitRefs[3];
...
SplitData Analysis::compressInterCU_rd0_4(const CUData& parentCTU, const CUGeom& cuGeom, int32_t qp)
{
            ...
            for (int i = 0; i < m_frame->m_encData->m_slice->m_numRefIdx[list]; i++) {
                for (int planes = 0; planes < INTEGRAL_PLANE_NUM; planes++) {
                    m_modeDepth[depth].fencYuv.m_integral[list][i][planes] =
                        m_frame->m_encData->m_slice->m_refFrameList[list][i]->m_encData->m_meIntegral[planes] + offset;
                }
            }
        }
    }
    ...
    if (bHEVCBlockAnalysis || bRefineAVCAnalysis || bNooffloading) {
        ...
        /* TODO: Re-evaluate if analysis load/save still works */
        {
            ...
            checkMerge2Nx2N_rd0_4(md.pred[PRED_SKIP], md.pred[PRED_MERGE], cuGeom);
            if (m_param->rdLevel) {
                skipModes = (m_param->bEnableEarlySkip || m_refineLevel == 2) && md.bestMode &&
                ...
                if (!skipRectAmp) {
                    if (m_param->bEnableRectInter) {
                        ...
                        if (m_slice->m_sliceType == P_SLICE) {
                            ...
                            threshold_2NxN =
                                (splitData[0].mvCost[0] + splitData[1].mvCost[0] + splitData[0].mvCost[1] + splitData[1].mvCost[1] + 1) >> 1;
                            threshold_Nx2N =
                                (splitData[0].mvCost[0] + splitData[2].mvCost[0] + splitData[0].mvCost[1] + splitData[2].mvCost[1] + 1) >> 1;
                        }
                    ...
                    if (m_slice->m_sps->maxAMPDepth > depth) {
                        ...
                        if (m_slice->m_sliceType == P_SLICE) {
                            ...
                            threshold_2NxnU =
                                (splitData[0].mvCost[0] + splitData[1].mvCost[0] + splitData[0].mvCost[1] + splitData[1].mvCost[1] + 1) >> 1;
                            threshold_2NxnD =
                                (splitData[2].mvCost[0] + splitData[3].mvCost[0] + splitData[2].mvCost[1] + splitData[3].mvCost[1] + 1) >> 1;

                            threshold_nLx2N =
                                (splitData[0].mvCost[0] + splitData[2].mvCost[0] + splitData[0].mvCost[1] + splitData[2].mvCost[1] + 1) >> 1;
                            threshold_nRx2N =
                                (splitData[1].mvCost[0] + splitData[3].mvCost[0] + splitData[1].mvCost[1] + splitData[3].mvCost[1] + 1) >> 1;
                        }
                        ...
                        if (bestInter->cu.m_partSize[0] == SIZE_2NxN) {
                            ...
                            bVer = true;
                        } else if (bestInter->cu.m_partSize[0] == SIZE_2Nx2N && md.bestMode && md.bestMode->cu.getQtRootCbf(0)) {
                            bHor = true;
                            bVer = true;
                        }
                ...
                if (m_param->rdLevel >= 3) {
                    ...
                    if ((!m_bChromaSa8d && (m_csp != X265_CSP_I400)) ||
                        (m_frame->m_fencPic->m_picCsp == X265_CSP_I400 &&
                         m_csp != X265_CSP_I400)) /* When m_bChromaSa8d is enabled, chroma MC has already been done */
                    {
                        uint32_t numPU = bestInter->cu.getNumPartInter(0);
                        for (uint32_t puIdx = 0; puIdx < numPU; puIdx++) {
                            PredictionUnit pu(bestInter->cu, cuGeom, puIdx);
...
#endif
{
    ...
    if (bHEVCBlockAnalysis || bRefineAVCAnalysis || bNooffloading) {
        ...
        if ((mightNotSplit && !md.bestMode && !bCtuInfoCheck) ||
            ...
            md.pred[PRED_MERGE].cu.initSubCU(parentCTU, cuGeom, qp);
            checkMerge2Nx2N_rd5_6(md.pred[PRED_SKIP], md.pred[PRED_MERGE], cuGeom);
            skipModes = (m_param->bEnableEarlySkip || m_refineLevel == 2) && md.bestMode && !md.bestMode->cu.getQtRootCbf(0);
            refMasks[0] = allSplitRefs;
            md.pred[PRED_2Nx2N].cu.initSubCU(parentCTU, cuGeom, qp);
            checkInter_rd5_6(md.pred[PRED_2Nx2N], cuGeom, SIZE_2Nx2N, refMasks);
            checkBestMode(md.pred[PRED_2Nx2N], cuGeom.depth);
        ...
        if (mightNotSplit) {
            ...
            if (!skipModes) {
                ...
                if (m_param->bEnableSCC) {
                    ...
                    if (intraBlockCopyFastSearch) {
                        if ((int)depth == m_slice->m_sps->log2DiffMaxMinCodingBlockSize) {
                            md.pred[PRED_IBC_Nx2N].cu.initSubCU(parentCTU, cuGeom, qp, ibc->m_lastIntraBCMv);
                            checkIntraBC_rd5_6(md.pred[PRED_IBC_Nx2N], cuGeom, SIZE_Nx2N, false, bUse1DSearchFor8x8, *ibc,
                                               (iMVCandList[SIZE_Nx2N] + 8));
                            checkBestMode(md.pred[PRED_IBC_Nx2N], depth);

                            md.pred[PRED_MIXED_IBC_NX2N].cu.initSubCU(parentCTU, cuGeom, qp, ibc->m_lastIntraBCMv);
                            bValid = predMixedIntraBCInterSearch(md.pred[PRED_MIXED_IBC_NX2N], cuGeom,
                                                                 m_csp != X265_CSP_I400 && m_frame->m_fencPic->m_picCsp != X265_CSP_I400, SIZE_Nx2N,
                            ...
                            md.pred[PRED_IBC_2NxN].cu.initSubCU(parentCTU, cuGeom, qp, ibc->m_lastIntraBCMv);
                            checkIntraBC_rd5_6(md.pred[PRED_IBC_2NxN], cuGeom, SIZE_2NxN, false, bUse1DSearchFor8x8, *ibc,
                                               (iMVCandList[SIZE_2NxN] + 8));
                            checkBestMode(md.pred[PRED_IBC_2NxN], depth);

                            md.pred[PRED_MIXED_IBC_2NXN].cu.initSubCU(parentCTU, cuGeom, qp, ibc->m_lastIntraBCMv);
                            bValid = predMixedIntraBCInterSearch(md.pred[PRED_MIXED_IBC_2NXN], cuGeom,
            ...
            if ((md.bestMode->cu.isInter(0) && !(md.bestMode->cu.m_mergeFlag[0] && md.bestMode->cu.m_partSize[0] == SIZE_2Nx2N)) &&
                (m_frame->m_fencPic->m_picCsp == X265_CSP_I400 && m_csp != X265_CSP_I400)) {
                uint32_t numPU = md.bestMode->cu.getNumPartInter(0);
...
void Analysis::recodeCU(const CUData& parentCTU, const CUGeom& cuGeom, int32_t qp, int32_t lqp)
{
    ...
    if (m_param->interRefine == 1) {
        split = (m_param->scaleFactor && bDecidedDepth && parentCTU.m_predMode[cuGeom.absPartIdx] == MODE_SKIP &&
                 (!mightNotSplit || (m_refineLevel && cuGeom.log2CUSize == (uint32_t)(g_log2Size[m_param->minCUSize] + 1))));
    } else {
    ...
    if ((bDecidedDepth && mightNotSplit) || (m_param->bAnalysisType == HEVC_INFO && parentCTU.m_cuDepth[cuGeom.absPartIdx] == 4)) {
        ...
        if (parentCTU.isIntra(cuGeom.absPartIdx) && m_refineLevel < 2) {
            if (m_param->intraRefine == 4) {
                ...
                bool reuseModes =
                    !((m_param->intraRefine == 3) || (m_param->intraRefine == 2 && parentCTU.m_lumaIntraDir[cuGeom.absPartIdx] > DC_IDX));
                if (reuseModes) {
                    memcpy(mode.cu.m_lumaIntraDir, parentCTU.m_lumaIntraDir + cuGeom.absPartIdx, cuGeom.numPartitions);
                    memcpy(mode.cu.m_chromaIntraDir, parentCTU.m_chromaIntraDir + cuGeom.absPartIdx, cuGeom.numPartitions);
                }
            ...
            for (uint32_t part = 0; part < numPU; part++) {
                ...
                if (m_param->analysisLoadReuseLevel == 10 || (m_param->bAnalysisType == AVC_INFO && m_param->analysisLoadReuseLevel >= 7)) {
                    ...
                    if (!mode.cu.m_mergeFlag[pu.puAbsPartIdx]) {
                        if (m_param->interRefine == 1) {
                            m_me.setSourcePU(*mode.fencYuv, pu.ctuAddr, pu.cuAbsPartIdx, pu.puAbsPartIdx, pu.width, pu.height, m_param->searchMethod,
                        ...
                        for (int list = 0; list < m_slice->isInterB() + 1; list++) {
                            ...
                            mode.cu.m_mvd[list][pu.puAbsPartIdx] =
                                mode.cu.m_mv[list][pu.puAbsPartIdx] - mode.amvpCand[list][ref][mode.cu.m_mvpIdx[list][pu.puAbsPartIdx]] /*mvp*/;
                        }
                    } else {
...
void Analysis::trainCU(const CUData& ctu, const CUGeom& cuGeom, const Mode& bestMode, TrainingData& trainData)
{
    ...
    if (!m_frame->m_classifyFrame) {
        ...
        bool refineInter0 = (trainData.predMode == ctu.m_predMode[cuGeom.absPartIdx] && trainData.partSize == ctu.m_partSize[cuGeom.absPartIdx] &&
...
void Analysis::checkMerge2Nx2N_rd0_4(Mode& skip, Mode& merge, const CUGeom& cuGeom)
{
    ...
    for (uint32_t i = 0; i < numMergeCand; ++i) {
        if (m_bFrameParallel) {
            ...
            if (candMvField[i][0].mv.y >= (m_param->searchRange + 1) * 4 || candMvField[i][1].mv.y >= (m_param->searchRange + 1) * 4) {
                continue;
            }
        }

        if (m_param->bIntraRefresh && m_slice->m_sliceType == P_SLICE &&
            tempPred->cu.m_cuPelX / m_param->maxCUSize < m_frame->m_encData->m_pir.pirEndCol && candMvField[i][0].mv.x > maxSafeMv) {
        ...
        motionCompensation(tempPred->cu, pu, tempPred->predYuv, true,
                           m_bChromaSa8d && (m_csp != X265_CSP_I400 && m_frame->m_fencPic->m_picCsp != X265_CSP_I400));
        ...
        if (m_bChromaSa8d && (m_csp != X265_CSP_I400 && m_frame->m_fencPic->m_picCsp != X265_CSP_I400)) {
            tempPred->distortion +=
                primitives.chroma[m_csp].cu[sizeIdx].sa8d(fencYuv->m_buf[1], fencYuv->m_csize, tempPred->predYuv.m_buf[1], tempPred->predYuv.m_csize);
            tempPred->distortion +=
                primitives.chroma[m_csp].cu[sizeIdx].sa8d(fencYuv->m_buf[2], fencYuv->m_csize, tempPred->predYuv.m_buf[2], tempPred->predYuv.m_csize);
        }
    ...
    if ((!m_bChromaSa8d && (m_csp != X265_CSP_I400)) ||
        (m_frame->m_fencPic->m_picCsp == X265_CSP_I400 && m_csp != X265_CSP_I400)) { /* Chroma MC was done above */
...
void Analysis::checkMerge2Nx2N_rd5_6(Mode& skip, Mode& merge, const CUGeom& cuGeom)
{
    ...
    for (uint32_t i = 0; i < numMergeCand; i++) {
        ...
        if (candDir[i] == 1 && !candMvField[i][0].mv.word && !candMvField[i][0].refIdx) {
            ...
            triedPZero = true;
        } else if (candDir[i] == 3 && !candMvField[i][0].mv.word && !candMvField[i][0].refIdx && !candMvField[i][1].mv.word &&
                   !candMvField[i][1].refIdx) {
            if (triedBZero) {
                continue;
            }
            triedBZero = true;
...
void Analysis::checkRDCostIntraBCMerge2Nx2N(Mode& mergeIBC, const CUGeom& cuGeom)
{
    ...
    for (uint8_t mergeCand = 0; mergeCand < numValidMergeCand; ++mergeCand) {
        ...
        if (!isBlockVectorValid(xPos, yPos, width, height, &cu, 0, 0, (cMvFieldNeighbours[mergeCand][0].mv.x >> 2),
                                (cMvFieldNeighbours[mergeCand][0].mv.y >> 2), m_param->maxCUSize)) {
            continue;
        }
...
#if ENABLE_SCC_EXT
void Analysis::checkIntraBC_rd5_6(Mode& intraBCMode, const CUGeom& cuGeom, PartSize ePartSize, bool testOnlyPred, bool bUse1DSearchFor8x8, IBC& ibc,
                                  MV* iMVCandList)
{
    ...
    bool bValid = predIntraBCSearch(intraBCMode, cuGeom, m_csp != X265_CSP_I400 && m_frame->m_fencPic->m_picCsp != X265_CSP_I400, ePartSize,
...
void Analysis::checkBidir2Nx2N(Mode& inter2Nx2N, Mode& bidir2Nx2N, const CUGeom& cuGeom)
{
...
#if ENABLE_SCC_EXT
    if ((cu.is8x8BipredRestriction(inter2Nx2N.bestME[0][0].mv, inter2Nx2N.bestME[0][1].mv, inter2Nx2N.bestME[0][0].ref, inter2Nx2N.bestME[0][1].ref)
             ? (1 << cu.m_log2CUSize[0] == 8)
             : cu.isBipredRestriction()) ||
        inter2Nx2N.bestME[0][0].cost == MAX_UINT || inter2Nx2N.bestME[0][1].cost == MAX_UINT)
#else
    ...
    if (m_bChromaSa8d && (m_csp != X265_CSP_I400 && m_frame->m_fencPic->m_picCsp != X265_CSP_I400)) {
        ...
        sa8d +=
            primitives.chroma[m_csp].cu[partEnum].sa8d(fencYuv.m_buf[1], fencYuv.m_csize, bidir2Nx2N.predYuv.m_buf[1], bidir2Nx2N.predYuv.m_csize);
        sa8d +=
            primitives.chroma[m_csp].cu[partEnum].sa8d(fencYuv.m_buf[2], fencYuv.m_csize, bidir2Nx2N.predYuv.m_buf[2], bidir2Nx2N.predYuv.m_csize);
    }
    ...
    if (bTryZero) {
        ...
        if (m_bChromaSa8d && (m_csp != X265_CSP_I400 && m_frame->m_fencPic->m_picCsp != X265_CSP_I400)) {
            ...
            primitives.pu[partEnum].pixelavg_pp[(tmpPredYuv.m_size % 64 == 0) && (refStride % 64 == 0)](tmpPredYuv.m_buf[0], tmpPredYuv.m_size, fref0,
                                                                                                        refStride, fref1, refStride, 32);
            zsa8d = primitives.cu[partEnum].sa8d(fencYuv.m_buf[0], fencYuv.m_size, tmpPredYuv.m_buf[0], tmpPredYuv.m_size);
        }
...
void Analysis::encodeResidue(const CUData& ctu, const CUGeom& cuGeom)
{
    ...
    } else  // if (cu.isInter(0))
    {
        ...
        primitives.cu[sizeIdx].sub_ps(resiYuv.m_buf[0], resiYuv.m_size, fencYuv.m_buf[0], predY, fencYuv.m_size, predYuv.m_size);

        if (m_csp != X265_CSP_I400 && m_frame->m_fencPic->m_picCsp != X265_CSP_I400) {
            pixel* predU = predYuv.getCbAddr(absPartIdx);
            pixel* predV = predYuv.getCrAddr(absPartIdx);
            primitives.chroma[m_csp].cu[sizeIdx].sub_ps(resiYuv.m_buf[1], resiYuv.m_csize, fencYuv.m_buf[1], predU, fencYuv.m_csize, predYuv.m_csize);

            primitives.chroma[m_csp].cu[sizeIdx].sub_ps(resiYuv.m_buf[2], resiYuv.m_csize, fencYuv.m_buf[2], predV, fencYuv.m_csize, predYuv.m_csize);
        }
        ...
        if (cu.m_cbf[0][0]) {
            ...
            primitives.cu[sizeIdx]
                .add_ps[reconPicAlign && predYalign && (reconPic.m_stride % 64 == 0) && (predYuv.m_size % 64 == 0) && (resiYuv.m_size % 64 == 0)](
                    reconPic.getLumaAddr(cu.m_cuAddr, absPartIdx), reconPic.m_stride, predY, resiYuv.m_buf[0], predYuv.m_size, resiYuv.m_size);
        } else {
            primitives.cu[sizeIdx].copy_pp(reconPic.getLumaAddr(cu.m_cuAddr, absPartIdx), reconPic.m_stride, predY, predYuv.m_size);
        }
        if (m_csp != X265_CSP_I400 && m_frame->m_fencPic->m_picCsp != X265_CSP_I400) {
            pixel* predU = predYuv.getCbAddr(absPartIdx);
            pixel* predV = predYuv.getCrAddr(absPartIdx);
            if (cu.m_cbf[1][0]) {
                ...
                primitives.chroma[m_csp].cu[sizeIdx].add_ps[reconPicAlign && predUalign && (reconPic.m_strideC % 64 == 0) &&
                                                            (predYuv.m_csize % 64 == 0) && (resiYuv.m_csize % 64 == 0)](
                    reconPic.getCbAddr(cu.m_cuAddr, absPartIdx), reconPic.m_strideC, predU, resiYuv.m_buf[1], predYuv.m_csize, resiYuv.m_csize);
            } else {
                primitives.chroma[m_csp].cu[sizeIdx].copy_pp(reconPic.getCbAddr(cu.m_cuAddr, absPartIdx), reconPic.m_strideC, predU, predYuv.m_csize);
            }

            if (cu.m_cbf[2][0]) {
                bool reconPicAlign = (reconPic.m_cuOffsetC[cu.m_cuAddr] + reconPic.m_buOffsetC[absPartIdx]) % 64 == 0;
                bool predValign = predYuv.getChromaAddrOffset(absPartIdx) % 64 == 0;
                primitives.chroma[m_csp].cu[sizeIdx].add_ps[reconPicAlign && predValign && (reconPic.m_strideC % 64 == 0) &&
                                                            (predYuv.m_csize % 64 == 0) && (resiYuv.m_csize % 64 == 0)](
                    reconPic.getCrAddr(cu.m_cuAddr, absPartIdx), reconPic.m_strideC, predV, resiYuv.m_buf[2], predYuv.m_csize, resiYuv.m_csize);
            } else {
                primitives.chroma[m_csp].cu[sizeIdx].copy_pp(reconPic.getCrAddr(cu.m_cuAddr, absPartIdx), reconPic.m_strideC, predV, predYuv.m_csize);
            }
        }
    }
    ...
    if (m_param->analysisLoadReuseLevel >= 2 && m_param->rc.cuTree) {
        ...
        if (ctu.m_slice->m_sliceType == I_SLICE) {
            return x265_clip3(m_param->rc.qpMin, m_param->rc.qpMax,
                              (int32_t)(qp + 0.5 + ((x265_analysis_intra_data*)m_frame->m_analysisData.intraData)->cuQPOff[cuIdx]));
        } else {
            return x265_clip3(m_param->rc.qpMin, m_param->rc.qpMax,
                              (int32_t)(qp + 0.5 + ((x265_analysis_inter_data*)m_frame->m_analysisData.interData)->cuQPOff[cuIdx]));
        }
    }
...
int Analysis::findSameContentRefCount(const CUData& parentCTU, const CUGeom& cuGeom)
{
    ...
    for (int list = 0; list < numPredDir; list++) {
        for (int i = 0; i < m_frame->m_encData->m_slice->m_numRefIdx[list]; i++) {
            ...
            if ((refPoc < prevChange && refPoc < m_curPoc) || (refPoc > m_curPoc && prevChange < m_curPoc && refPrevChange > m_curPoc) ||
                ((refPoc == prevChange) && (m_additionalCtuInfo[cuGeom.absPartIdx] == CTU_INFO_CHANGE))) {
