    ...
    void writeSEI(const SPS&)
    {
        ...
        if (!m_filmGrainCharacteristicsCancelFlag) {
            ...
            for (uint8_t c = 0; c < 3; c++) {
                WRITE_FLAG(
                    m_compModel[c].bPresentFlag && m_compModel[c].m_filmGrainNumIntensityIntervalMinus1 + 1 > 0 && m_compModel[c].numModelValues > 0,
                    "comp_model_present_flag[c]");
            }
            for (uint8_t c = 0; c < 3; c++) {
                if (m_compModel[c].bPresentFlag && m_compModel[c].m_filmGrainNumIntensityIntervalMinus1 + 1 > 0 &&
                    ...
                    for (uint8_t interval = 0; interval < m_compModel[c].m_filmGrainNumIntensityIntervalMinus1 + 1; interval++) {
                        WRITE_CODE(m_compModel[c].intensityValues[interval].intensityIntervalLowerBound, X265_BYTE,
                                   "intensity_interval_lower_bound[c][i]");
                        WRITE_CODE(m_compModel[c].intensityValues[interval].intensityIntervalUpperBound, X265_BYTE,
                                   "intensity_interval_upper_bound[c][i]");
...
public:
    SEIThreeDimensionalReferenceDisplaysInfo() : m_numRefDisplaysMinus1(0), m_refViewingDistanceFlag(false), m_additionalShiftPresentFlag(false)
    {
...
static int temp_sign_r[3][3] = {
    {0, 1, 0},
    {1, 0, 0},
    {0, 1, 1}
};
static int temp_exponent_r[3][3] = {
    {10, 20, 11},
    {10,  5, 11},
    { 2, 20, 11}
};
static int temp_mantissa_r[3][3] = {
    {4, 9, 1},
    {0, 3, 4},
    {3, 3, 7}
};
static int temp_sign_t[1][3] = {
    {0, 1, 0}
};
static int temp_exponent_t[1][3] = {
    {0, 10, 5}
};
static int temp_mantissa_t[1][3] = {
    {1, 8, 9}
};
static int temp_length_mantissa_r[3][3] = {
    {10, 20, 11},
    {10,  5, 11},
    { 2, 20, 11}
};
static int temp_length_mantissa_t[1][3] = {
    {1, 10, 5}
};
...
public:
    ...
    {
        return sscanf(value, "G(%hu,%hu)B(%hu,%hu)R(%hu,%hu)WP(%hu,%hu)L(%u,%u)", &displayPrimaryX[0], &displayPrimaryY[0], &displayPrimaryX[1],
                      &displayPrimaryY[1], &displayPrimaryX[2], &displayPrimaryY[2], &whitePointX, &whitePointY, &maxDisplayMasteringLuminance,
...
public:
    SEIBufferingPeriod() : m_cpbDelayOffset(0), m_dpbDelayOffset(0), m_concatenationFlag(0), m_auCpbRemovalDelayDelta(1)
    {
...
public:
    ...
    void writeSEI(const SPS&) { WRITE_CODE(m_preferredTransferCharacteristics, 8, "Preferred transfer characteristics"); }
};
