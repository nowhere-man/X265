...
#if ENABLE_MULTIVIEW
            if (curFrame->m_param->numViews > 1 && !curFrame->m_viewId && m_picList.getPOC(curFrame->m_poc, 1) &&
                curFrame == m_picList.getPOC(curFrame->m_poc, 1)->refPicSetInterLayer0.getPOC(curFrame->m_poc, curFrame->m_viewId)) {
...
void DPB::prepareEncode(Frame* newFrame)
{
    ...
    if (slice->m_param->bEnableSCC) {
        slice->m_origSliceType = slice->m_sliceType;
    }
    ...
    uint32_t maxDecBuffer = (slice->m_sps->maxDecPicBuffering[newFrame->m_tempLayer] >= 8 && slice->m_param->bEnableSCC)
                                ? 7
    ...
    if (m_bTemporalSublayer && newFrame->m_tempLayer > 0 &&
        !(slice->m_nalUnitType == NAL_UNIT_CODED_SLICE_RADL_N  // Check if not a leading picture
          || slice->m_nalUnitType == NAL_UNIT_CODED_SLICE_RADL_R || slice->m_nalUnitType == NAL_UNIT_CODED_SLICE_RASL_N ||
    ...
    if (slice->m_sliceType != I_SLICE) {
        slice->m_numRefIdx[0] = x265_clip3(1, newFrame->m_param->maxNumReferences,
                                           numRef + newFrame->refPicSetInterLayer0.size() + newFrame->refPicSetInterLayer1.size());
    } else {
    ...
    if (slice->m_param->numViews > 1 || !!slice->m_param->bEnableSCC) {
        slice->m_numRefIdx[1] =
            X265_MIN(newFrame->m_param->bBPyramid ? 3 : 2,
                     slice->m_rps.numberOfPositivePictures + newFrame->refPicSetInterLayer0.size() + newFrame->refPicSetInterLayer1.size());
    } else
#endif
...
#if ENABLE_MULTIVIEW
                if (iterPic->m_param->numViews > 1 && layer && numNeg == (uint8_t)(iterPic->m_param->maxNumReferences - 1) &&
                    (iterPic->m_poc - curPoc) < 0) {
    ...
    if (nalUnitType == NAL_UNIT_CODED_SLICE_IDR_W_RADL || nalUnitType == NAL_UNIT_CODED_SLICE_IDR_N_LP) {
        ...
        while (iterFrame) {
            int layer = iterFrame->m_param->numViews > 1              ? iterFrame->m_viewId
                        : (iterFrame->m_param->numScalableLayers > 1) ? iterFrame->m_sLayerId
                                                                      : 0;
