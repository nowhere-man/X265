...
Y4MOutput::Y4MOutput(const char *filename, int w, int h, uint32_t bitdepth, uint32_t fpsNum, uint32_t fpsDenom, int csp, int inputdepth)
    : width(w), height(h), bitDepth(bitdepth), colorSpace(csp), frameSize(0), inputDepth(inputdepth)
{
    ...
    if (ofs) {
        if (bitDepth == 10) {
            ofs << "YUV4MPEG2 W" << width << " H" << height << " F" << fpsNum << ":" << fpsDenom << " Ip" << " C" << cf << "p10" << " XYSCSS = " << cf
                << "P10" << "\n";
        } else if (bitDepth == 12) {
            ofs << "YUV4MPEG2 W" << width << " H" << height << " F" << fpsNum << ":" << fpsDenom << " Ip" << " C" << cf << "p12" << " XYSCSS = " << cf
                << "P12" << "\n";
        } else {
