...
bool IntraPredHarness::check_allangs_primitive(const intra_allangs_t ref, const intra_allangs_t opt, int sizeIdx)
{
    ...
    for (int i = 0; i <= 100; i++) {
        ...
        for (int p = 2 - 2; p <= 34 - 2; p++) {
            for (int k = 0; k < width; k++) {
                if (memcmp(pixel_out_33_c + p * (width * width) + k * width, pixel_out_33_vec + p * (width * width) + k * width,
                           width * sizeof(pixel))) {
...
void IntraPredHarness::measureSpeed(const EncoderPrimitives& ref, const EncoderPrimitives& opt)
{
    ...
    for (int i = BLOCK_4x4; i <= BLOCK_32x32; i++) {
        ...
        if (opt.cu[i].intra_pred[PLANAR_IDX]) {
            ...
            REPORT_SPEEDUP(opt.cu[i].intra_pred[PLANAR_IDX], ref.cu[i].intra_pred[PLANAR_IDX], pixel_out_vec, FENC_STRIDE, pixel_buff + srcStride, 0,
                           0);
        }
        if (opt.cu[i].intra_pred[DC_IDX]) {
            printf("intra_dc_%dx%d[f=0]", size, size);
            REPORT_SPEEDUP(opt.cu[i].intra_pred[DC_IDX], ref.cu[i].intra_pred[DC_IDX], pixel_out_vec, FENC_STRIDE, pixel_buff + srcStride, 0, 0);
            if (size <= 16) {
                printf("intra_dc_%dx%d[f=1]", size, size);
                REPORT_SPEEDUP(opt.cu[i].intra_pred[DC_IDX], ref.cu[i].intra_pred[DC_IDX], pixel_out_vec, FENC_STRIDE, pixel_buff + srcStride, 0, 1);
            }
        }
        if (opt.cu[i].intra_pred_allangs) {
            ...
            REPORT_SPEEDUP(opt.cu[i].intra_pred_allangs, ref.cu[i].intra_pred_allangs, pixel_out_33_vec, refAbove, refLeft, bFilter);
        }
        for (int mode = 2; mode <= 34; mode += 1) {
            if (opt.cu[i].intra_pred[mode]) {
                ...
                REPORT_SPEEDUP(opt.cu[i].intra_pred[mode], ref.cu[i].intra_pred[mode], pixel_out_vec, FENC_STRIDE, pixel_buff + srcStride, mode,
                               bFilter);
            }
        }
        if (opt.cu[i].intra_filter) {
