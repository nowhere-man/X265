...
bool MBDstHarness::check_quant_primitive(quant_t ref, quant_t opt)
{
    ...
    for (int i = 0; i < ITERS; i++) {
        ...
        optReturnValue =
            (uint32_t)checked(opt, short_test_buff[index1] + j, int_test_buff[index2] + j, mintbuf3, mshortbuf3, bits, valueToAdd, numCoeff);
...
void MBDstHarness::measureSpeed(const EncoderPrimitives& ref, const EncoderPrimitives& opt)
{
    ...
    for (int value = 0; value < NUM_TR_SIZE; value++) {
        if (opt.cu[value].nonPsyRdoQuant) {
            ...
            REPORT_SPEEDUP(opt.cu[value].nonPsyRdoQuant, ref.cu[value].nonPsyRdoQuant, short_test_buff[0], opt_dest, &totalUncodedCost, &totalRdCost,
                           0);
        }
    }
    for (int value = 0; value < NUM_TR_SIZE; value++) {
        if (opt.cu[value].psyRdoQuant) {
            ...
            *psyScale = 0;
            ...
            REPORT_SPEEDUP(opt.cu[value].psyRdoQuant, ref.cu[value].psyRdoQuant, short_test_buff[0], short_test_buff1[0], opt_dest, &totalUncodedCost,
                           &totalRdCost, psyScale, 0);
        }
    }
    for (int value = 0; value < NUM_TR_SIZE; value++) {
        if (opt.cu[value].psyRdoQuant_1p) {
            ...
            REPORT_SPEEDUP(opt.cu[value].psyRdoQuant_1p, ref.cu[value].psyRdoQuant_1p, short_test_buff[0], opt_dest, &totalUncodedCost, &totalRdCost,
                           0);
        }
    }
    for (int value = 0; value < NUM_TR_SIZE; value++) {
