...
bool PixelHarness::check_pixelcmp_x3(pixelcmp_x3_t ref, pixelcmp_x3_t opt)
{
    ...
    for (int i = 0; i < ITERS; i++) {
        int index1 = rand() % TEST_CASES;
        int index2 = rand() % TEST_CASES;
        checked(opt, pixel_test_buff[index1], pixel_test_buff[index2] + j, pixel_test_buff[index2] + j + 1, pixel_test_buff[index2] + j + 2, stride,
                &vres[0]);
        ref(pixel_test_buff[index1], pixel_test_buff[index2] + j, pixel_test_buff[index2] + j + 1, pixel_test_buff[index2] + j + 2, stride, &cres[0]);
...
bool PixelHarness::check_pixelcmp_x4(pixelcmp_x4_t ref, pixelcmp_x4_t opt)
{
    ...
    for (int i = 0; i < ITERS; i++) {
        int index1 = rand() % TEST_CASES;
        int index2 = rand() % TEST_CASES;
        checked(opt, pixel_test_buff[index1], pixel_test_buff[index2] + j, pixel_test_buff[index2] + j + 1, pixel_test_buff[index2] + j + 2,
                pixel_test_buff[index2] + j + 3, stride, &vres[0]);
        ref(pixel_test_buff[index1], pixel_test_buff[index2] + j, pixel_test_buff[index2] + j + 1, pixel_test_buff[index2] + j + 2,
...
bool PixelHarness::check_downscale_t(downscale_t ref, downscale_t opt)
{
    ...
    for (int i = 0; i < ITERS; i++) {
        int index = i % TEST_CASES;
        ref(pixel_test_buff[index] + j, ref_destf, ref_desth, ref_destv, ref_destc, src_stride, dst_stride, bx, by);
        checked(opt, pixel_test_buff[index] + j, opt_destf, opt_desth, opt_destv, opt_destc, src_stride, dst_stride, bx, by);
...
bool PixelHarness::check_pixelavg_pp(pixelavg_pp_t ref, pixelavg_pp_t opt)
{
    ...
    for (int i = 0; i < ITERS; i++) {
        int index1 = rand() % TEST_CASES;
        int index2 = rand() % TEST_CASES;
        checked(ref, ref_dest, stride, pixel_test_buff[index1] + j, stride, pixel_test_buff[index2] + j, stride, 32);
        opt(opt_dest, stride, pixel_test_buff[index1] + j, stride, pixel_test_buff[index2] + j, stride, 32);
...
bool PixelHarness::check_pixel_sub_ps(pixel_sub_ps_t ref, pixel_sub_ps_t opt)
{
    ...
    for (int i = 0; i < 1; i++) {
        int index1 = rand() % TEST_CASES;
        int index2 = rand() % TEST_CASES;
        checked(opt, opt_dest, stride2, pixel_test_buff[index1] + j, pixel_test_buff[index2] + j, stride, stride);
        ref(ref_dest, stride2, pixel_test_buff[index1] + j, pixel_test_buff[index2] + j, stride, stride);
...
bool PixelHarness::check_cutree_propagate_cost(cutree_propagate_cost ref, cutree_propagate_cost opt)
{
    ...
    for (int i = 0; i < ITERS; i++) {
        ...
        int index = i % TEST_CASES;
        checked(opt, opt_dest, ushort_test_buff[index] + j, int_test_buff[index] + j, ushort_test_buff[index] + j, int_test_buff[index] + j, &fps,
                width);
...
bool PixelHarness::check_costCoeffNxN(costCoeffNxN_t ref, costCoeffNxN_t opt)
{
    ...
    for (int i = 0; i < ITERS * 2; i++) {
        ...
        uint32_t ref_sum = ref(scanTblCG4x4, &ref_src[blkPosBase + i], (intptr_t)trSize, ref_absCoeff + numNonZero, rand_tabSigCtx, scanFlagMask,
                               (uint8_t*)ref_baseCtx, offset, rand_scanPosSigOff, subPosBase);
        uint32_t opt_sum = (uint32_t)checked(opt, scanTblCG4x4, &ref_src[blkPosBase + i], (intptr_t)trSize, opt_absCoeff + numNonZero, rand_tabSigCtx,
                                             scanFlagMask, (uint8_t*)opt_baseCtx, offset, rand_scanPosSigOff, subPosBase);
        ...
        if (opt_absCoeff[(1 << MLS_CG_SIZE) + 1] != 0x4567 || opt_absCoeff[(1 << MLS_CG_SIZE) + 2] != 0xBA98 ||
            opt_absCoeff[(1 << MLS_CG_SIZE) + 3] != 0xFEDC) {
            return false;
        }
...
bool PixelHarness::check_costC1C2Flag(costC1C2Flag_t ref, costC1C2Flag_t opt)
{
    ...
    for (int k = 0; k < 8; k++) {
        ref_baseCtx[k] = opt_baseCtx[k] = (rand() % (125 - 2)) + 2;
    }
...
void PixelHarness::measurePartition(int part, const EncoderPrimitives& ref, const EncoderPrimitives& opt)
{
    ...
    char header[128];
#define HEADER(str, ...)                                \
    snprintf(header, sizeof(header), str, __VA_ARGS__); \
    printf("%22s", header);
    ...
    for (int i = 0; i < X265_CSP_COUNT; i++) {
        ...
        if (opt.chroma[i].pu[part].addAvg[NONALIGNED]) {
            ...
            REPORT_SPEEDUP(opt.chroma[i].pu[part].addAvg[NONALIGNED], ref.chroma[i].pu[part].addAvg[NONALIGNED], sbuf1, sbuf2, pbuf1, STRIDE, STRIDE,
                           STRIDE);
        }
        if (opt.chroma[i].pu[part].addAvg[ALIGNED]) {
            HEADER("[%s]  addAvg_aligned[%s]", x265_source_csp_names[i], chromaPartStr[i][part]);
            REPORT_SPEEDUP(opt.chroma[i].pu[part].addAvg[ALIGNED], ref.chroma[i].pu[part].addAvg[ALIGNED], sbuf1, sbuf2, pbuf1, STRIDE, STRIDE,
                           STRIDE);
        }
        if (opt.chroma[i].pu[part].satd) {
        ...
        if (part < NUM_CU_SIZES) {
            ...
            if (opt.chroma[i].cu[part].sub_ps) {
                ...
                REPORT_SPEEDUP(opt.chroma[i].cu[part].sub_ps, ref.chroma[i].cu[part].sub_ps, (int16_t*)pbuf1, FENC_STRIDE, pbuf2, pbuf1, STRIDE,
                               STRIDE);
            }
            if (opt.chroma[i].cu[part].add_ps[NONALIGNED]) {
                HEADER("[%s]  add_ps[%s]", x265_source_csp_names[i], chromaPartStr[i][part]);
                REPORT_SPEEDUP(opt.chroma[i].cu[part].add_ps[NONALIGNED], ref.chroma[i].cu[part].add_ps[NONALIGNED], pbuf1, FENC_STRIDE, pbuf2, sbuf1,
                               STRIDE, STRIDE);
            }
            if (opt.chroma[i].cu[part].add_ps[ALIGNED]) {
                HEADER("[%s]  add_ps_aligned[%s]", x265_source_csp_names[i], chromaPartStr[i][part]);
                REPORT_SPEEDUP(opt.chroma[i].cu[part].add_ps[ALIGNED], ref.chroma[i].cu[part].add_ps[ALIGNED], pbuf1, FENC_STRIDE, pbuf2, sbuf1,
                               STRIDE, STRIDE);
            }
            if (opt.chroma[i].cu[part].sa8d) {
...
#define HEADER0(str) printf("%22s", str);
    ...
    if (opt.weight_pp) {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            REPORT_SPEEDUP(opt.weight_pp, ref.weight_pp, pbuf1, pbuf2, 64, 32, 32, w0[i], round << correction, shift + correction, offset);
        }
    }
    ...
    if (opt.planecopy_cp) {
        ...
        REPORT_SPEEDUP(opt.planecopy_cp, ref.planecopy_cp, uchar_test_buff[0], 64, pbuf1, 64, 64, 64, X265_DEPTH - 8);
    }

    if (opt.propagateCost) {
        HEADER0("propagateCost");
        REPORT_SPEEDUP(opt.propagateCost, ref.propagateCost, ibuf1, ushort_test_buff[0], int_test_buff[0], ushort_test_buff[0], int_test_buff[0],
    ...
    if (opt.scanPosLast) {
        ...
        REPORT_SPEEDUP(opt.scanPosLast, ref.scanPosLast, g_scanOrder[SCAN_DIAG][NUM_SCAN_SIZE - 1], coefBuf, (uint16_t*)sbuf1, (uint16_t*)sbuf2,
                       (uint8_t*)psbuf1, 32, g_scan4x4[SCAN_DIAG], 32);
    }
    ...
    if (opt.findPosFirstLast) {
        ...
        REPORT_SPEEDUP(opt.findPosFirstLast, ref.findPosFirstLast, coefBuf, trSize, g_scan4x4[SCAN_DIAG]);
        HEADER0("findPosFirstLast[SCAN_HOR]");
        REPORT_SPEEDUP(opt.findPosFirstLast, ref.findPosFirstLast, coefBuf, trSize, g_scan4x4[SCAN_HOR]);
        HEADER0("findPosFirstLast[SCAN_VER]");
        REPORT_SPEEDUP(opt.findPosFirstLast, ref.findPosFirstLast, coefBuf, trSize, g_scan4x4[SCAN_VER]);
    }
    ...
    if (opt.costCoeffNxN) {
        ...
        ALIGN_VAR_32(static uint8_t const, ctxSig[]) = {0, 1, 4, 5, 2, 3, 4, 5, 6, 6, 8, 8, 7, 7, 8, 8};
    ...
    if (opt.pelFilterLumaStrong[0]) {
        ...
        REPORT_SPEEDUP(opt.pelFilterLumaStrong[0], ref.pelFilterLumaStrong[0], pbuf1 + 4, STRIDE, 1, tcP, tcQ);
    }
    ...
    if (opt.pelFilterLumaStrong[1]) {
        ...
        REPORT_SPEEDUP(opt.pelFilterLumaStrong[1], ref.pelFilterLumaStrong[1], pbuf1 + 4 * STRIDE, 1, STRIDE, tcP, tcQ);
    }
    ...
    if (opt.pelFilterChroma[0]) {
        ...
        for (int i = 0; i < NUM_MASKS; i++) {
            ...
            REPORT_SPEEDUP(opt.pelFilterChroma[0], ref.pelFilterChroma[0], pbuf1 + 2, STRIDE, 1, tc, maskP, maskQ);
        }
    }
    ...
    if (opt.pelFilterChroma[1]) {
        ...
        for (int i = 0; i < NUM_MASKS; i++) {
            ...
            REPORT_SPEEDUP(opt.pelFilterChroma[1], ref.pelFilterChroma[1], pbuf1 + 2 * STRIDE, 1, STRIDE, tc, maskP, maskQ);
        }
    }
