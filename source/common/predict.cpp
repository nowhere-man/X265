...
void Predict::predInterLumaShort(const PredictionUnit& pu, ShortYuv& dstSYuv, const PicYuv& refPic, const MV& mv) const
{
    ...
    if (!(yFrac | xFrac)) {
        ...
        primitives.pu[partEnum].convert_p2s[srcStride % 64 == 0 && dstStride % 64 == 0 && srcbufferAlignCheck && dstbufferAlignCheck](src, srcStride,
                                                                                                                                      dst, dstStride);
    } else if (!yFrac) {
...
void Predict::predInterChromaShort(const PredictionUnit& pu, ShortYuv& dstSYuv, const PicYuv& refPic, const MV& mv) const
{
    ...
    if (!(yFrac | xFrac)) {
        ...
        primitives.chroma[m_csp].pu[partEnum].p2s[refStride % 64 == 0 && dstStride % 64 == 0 && srcbufferAlignCheckC && dstbufferAlignCheckC](
            refCb, refStride, dstCb, dstStride);
        primitives.chroma[m_csp].pu[partEnum].p2s[refStride % 64 == 0 && dstStride % 64 == 0 && srcbufferAlignCheckC && dstbufferAlignCheckC](
            refCr, refStride, dstCr, dstStride);
    } else if (!yFrac) {
...
void Predict::addWeightBi(const PredictionUnit& pu, Yuv& predYuv, const ShortYuv& srcYuv0, const ShortYuv& srcYuv1, const WeightValues wp0[3],
                          const WeightValues wp1[3], bool bLuma, bool bChroma) const
{
    int x, y;
...
void Predict::initAdiPattern(const CUData& cu, const CUGeom& cuGeom, uint32_t puAbsPartIdx, const IntraNeighbors& intraNeighbors, int dirMode)
{
    ...
    if (dirMode == ALL_IDX ? (8 | 16 | 32) & tuSize : g_intraFilterFlags[dirMode] & tuSize) {
        ...
        if (cu.m_slice->m_sps->bUseStrongIntraSmoothing && tuSize == 32) {
            ...
            if (abs(topLeft + topLast - (topMiddle << 1)) < threshold && abs(topLeft + leftLast - (leftMiddle << 1)) < threshold) {
...
void Predict::initAdiPatternChroma(const CUData& cu, const CUGeom& cuGeom, uint32_t puAbsPartIdx, const IntraNeighbors& intraNeighbors,
                                   uint32_t chromaId)
{
    PicYuv* reconPic = cu.m_encData->m_reconPic[0];
...
template <bool cip> bool Predict::isAboveLeftAvailable(const CUData& cu, uint32_t partIdxLT)
{
...
template <bool cip> int Predict::isAboveAvailable(const CUData& cu, uint32_t partIdxLT, uint32_t partIdxRT, bool* bValidFlags)
{
...
template <bool cip> int Predict::isLeftAvailable(const CUData& cu, uint32_t partIdxLT, uint32_t partIdxLB, bool* bValidFlags)
{
    const uint32_t rasterPartBegin = g_zscanToRaster[partIdxLT];
...
template <bool cip> int Predict::isAboveRightAvailable(const CUData& cu, uint32_t partIdxRT, bool* bValidFlags, uint32_t numUnits)
{
    int numIntra = 0;
...
template <bool cip> int Predict::isBelowLeftAvailable(const CUData& cu, uint32_t partIdxLB, bool* bValidFlags, uint32_t numUnits)
{
    int numIntra = 0;
