    ...
    for (y = 0; y < 2; y++) {
        ...
        if (width >= 8) {
            ...
            for (; (x + 8) <= width; x += 8) {
                ...
                t1 = vreinterpretq_s16_u16(vaddw_u8(vreinterpretq_u16_s16(t1), in));
...
void pelFilterLumaStrong_V_neon(pixel *src, intptr_t srcStep, intptr_t offset, int32_t tcP, int32_t tcQ)
{
    ...
    for (int i = 0; i < UNIT_SIZE; i++, src += srcStep) {
        ...
        sum = vsubw_u8(sum, s1);
        sum = vreinterpretq_u16_s16(vminq_s16(tc_vec, vmaxq_s16(neg_tc_vec, vreinterpretq_s16_u16(sum))));
...
void pelFilterLumaStrong_H_neon(pixel *src, intptr_t srcStep, intptr_t offset, int32_t tcP, int32_t tcQ)
{
    X265_CHECK(UNIT_SIZE == 4 && srcStep == 1, "UNIT_SIZE must be 4 and srcStep must be 1 for LumaStrong Horizontal\n");
    ...
    uint8x8_t m12 = vreinterpret_u8_u32(vzip1_u32(vreinterpret_u32_u8(m1), vreinterpret_u32_u8(m2)));
    uint8x8_t m23 = vreinterpret_u8_u32(vzip1_u32(vreinterpret_u32_u8(m2), vreinterpret_u32_u8(m3)));
    uint8x8_t m34 = vreinterpret_u8_u32(vzip1_u32(vreinterpret_u32_u8(m3), vreinterpret_u32_u8(m4)));
    uint8x8_t m45 = vreinterpret_u8_u32(vzip1_u32(vreinterpret_u32_u8(m4), vreinterpret_u32_u8(m5)));
    uint8x8_t m56 = vreinterpret_u8_u32(vzip1_u32(vreinterpret_u32_u8(m5), vreinterpret_u32_u8(m6)));
    ...
    t01 = vreinterpretq_u16_s16(vminq_s16(tc_vec, vmaxq_s16(neg_tc_vec, vreinterpretq_s16_u16(t01))));
    uint8x8_t d01 = vmovn_u16(t01);
    d01 = vadd_u8(d01, m34);
    store_u8x4_strided_xN<2>(&src[-1 * offset], 1 * offset, &d01);

    uint8x8_t m16 = vreinterpret_u8_u32(vzip1_u32(vreinterpret_u32_u8(m1), vreinterpret_u32_u8(m6)));
    uint8x8_t m25 = vreinterpret_u8_u32(vzip1_u32(vreinterpret_u32_u8(m2), vreinterpret_u32_u8(m5)));

    // src[-2 * offset], src[1 * offset]
    uint16x8_t p1 = vaddw_u8(p0, m16);
    uint16x8_t t23 = vrshrq_n_u16(p1, 2);
    t23 = vsubw_u8(t23, m25);
    t23 = vreinterpretq_u16_s16(vminq_s16(tc_vec, vmaxq_s16(neg_tc_vec, vreinterpretq_s16_u16(t23))));
    uint8x8_t d23 = vmovn_u16(t23);
    d23 = vadd_u8(d23, m25);
    store_u8x4_strided_xN<2>(&src[-2 * offset], 3 * offset, &d23);

    uint8x8_t m07 = vreinterpret_u8_u32(vzip1_u32(vreinterpret_u32_u8(m0), vreinterpret_u32_u8(m7)));
    ...
    t45 = vreinterpretq_u16_s16(vminq_s16(tc_vec, vmaxq_s16(neg_tc_vec, vreinterpretq_s16_u16(t45))));
