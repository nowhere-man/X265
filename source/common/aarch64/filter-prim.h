...
template <int coeffIdx> void inline filter8_s16x4(const int16x4_t *s, const int16x8_t filter, const int32x4_t c, int32x4_t &d)
{
    if (coeffIdx == 1) {
        d = vsubl_s16(s[6], s[0]);
        d = vaddq_s32(d, c);
...
template <int coeffIdx> void inline filter8_s16x8(const int16x8_t *s, const int16x8_t filter, const int32x4_t c, int32x4_t &d0, int32x4_t &d1)
{
    if (coeffIdx == 1) {
