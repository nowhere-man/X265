...
static inline void transpose_4x8_s16(int16x4_t s0, int16x4_t s1, int16x4_t s2, int16x4_t s3, int16x4_t s4, int16x4_t s5, int16x4_t s6, int16x4_t s7,
...
static int scanPosLast_opt(const uint16_t *scan, const coeff_t *coeff, uint16_t *coeffSign, uint16_t *coeffFlag, uint8_t *coeffNum, int numSig,
                           const uint16_t * /*scanCG4x4*/, const int /*trSize*/)
{
    // This is an optimized function for scanPosLast, which removes the rmw dependency, once integrated into mainline x265, should replace reference
    // implementation For clarity, left the original reference code in comments
    int scanPosLast = 0;
...
template <int log2TrSize>
static void psyRdoQuant_neon(int16_t *m_resiDctCoeff, int16_t *m_fencDctCoeff, int64_t *costUncoded, int64_t *totalUncodedCost, int64_t *totalRdCost,
...
template <int trSize> int count_nonzero_neon(const int16_t *quantCoeff)
{
...
template <int trSize> uint32_t copy_count_neon(int16_t *coeff, const int16_t *residual, intptr_t resiStride)
{
    uint32_t numSig = 0;
    int16x8_t vcount = vdupq_n_s16(0);
...
template <int shift> static inline void fastForwardDst4_neon(const int16_t *src, int16_t *dst)
{
...
template <int shift> static inline void inverseDst4_neon(const int16_t *src, int16_t *dst, intptr_t dstStride)
{
    int16x4_t s0 = vld1_s16(src + 0);
    int16x4_t s1 = vld1_s16(src + 4);
    int16x4_t s2 = vld1_s16(src + 8);
...
template <int shift> static inline void partialButterfly4_neon(const int16_t *src, int16_t *dst)
{
    int16x4_t s0 = vld1_s16(src + 0);
    int16x4_t s1 = vld1_s16(src + 4);
    int16x4_t s2 = vld1_s16(src + 8);
...
template <int shift> static inline void partialButterfly16_neon(const int16_t *src, int16_t *dst)
{
    const int line = 16;
...
template <int shift> static inline void partialButterfly32_neon(const int16_t *src, int16_t *dst)
{
    const int line = 32;
...
template <int shift> static inline void partialButterfly8_neon(const int16_t *src, int16_t *dst)
{
    const int line = 8;
    ...
    for (int j = 0; j < line; j += 4) {
        // O
        int32x4_t t01 = vpaddq_s32(vmull_s16(c1, O[j + 0]), vmull_s16(c1, O[j + 1]));
        int32x4_t t23 = vpaddq_s32(vmull_s16(c1, O[j + 2]), vmull_s16(c1, O[j + 3]));
...
template <int shift> static inline void partialButterflyInverse4_neon(const int16_t *src, int16_t *dst, intptr_t dstStride)
{
    int16x4_t s0 = vld1_s16(src + 0);
    int16x4_t s1 = vld1_s16(src + 4);
    int16x4_t s2 = vld1_s16(src + 8);
...
template <int shift> static inline void partialButterflyInverse8_neon(const int16_t *src, int16_t *dst, intptr_t dstStride)
{
    const int line = 8;
    ...
    transpose_4x8_s16(d_lo[0], d_lo[1], d_lo[2], d_lo[3], d_lo[4], d_lo[5], d_lo[6], d_lo[7], d0, d1, d2, d3);

    transpose_4x8_s16(d_hi[0], d_hi[1], d_hi[2], d_hi[3], d_hi[4], d_hi[5], d_hi[6], d_hi[7], d4, d5, d6, d7);
...
template <int shift> static inline void partialButterflyInverse16_neon(const int16_t *src, int16_t *dst, intptr_t dstStride)
{
    const int line = 16;
    ...
    for (int i = 0; i < 4; i++) {
        ...
        transpose_4x8_s16(d_lo[0], d_lo[1], d_lo[2], d_lo[3], d_lo[4], d_lo[5], d_lo[6], d_lo[7], d0_lo, d1_lo, d2_lo, d3_lo);
        transpose_4x8_s16(d_hi[0], d_hi[1], d_hi[2], d_hi[3], d_hi[4], d_hi[5], d_hi[6], d_hi[7], d0_hi, d1_hi, d2_hi, d3_hi);
...
template <int shift> static inline void partialButterflyInverse32_neon(const int16_t *src, int16_t *dst, intptr_t dstStride)
{
    const int line = 32;
    ...
    for (int i = 0; i < 8; i++) {
        ...
        int16x8_t d2[4];
        int16x8_t d3[4];
        transpose_4x8_s16(d_lo[0], d_lo[1], d_lo[2], d_lo[3], d_lo[4], d_lo[5], d_lo[6], d_lo[7], d0[0], d1[0], d2[0], d3[0]);
        transpose_4x8_s16(d_lo[8], d_lo[9], d_lo[10], d_lo[11], d_lo[12], d_lo[13], d_lo[14], d_lo[15], d0[1], d1[1], d2[1], d3[1]);
        transpose_4x8_s16(d_hi[0], d_hi[1], d_hi[2], d_hi[3], d_hi[4], d_hi[5], d_hi[6], d_hi[7], d0[2], d1[2], d2[2], d3[2]);
        transpose_4x8_s16(d_hi[8], d_hi[9], d_hi[10], d_hi[11], d_hi[12], d_hi[13], d_hi[14], d_hi[15], d0[3], d1[3], d2[3], d3[3]);
...
uint32_t findPosFirstLast_neon(const int16_t *coeff, const intptr_t trSize, const uint16_t scanTbl[16])
{
    ...
    uint64_t cmp_4bit = vget_lane_u64(vreinterpret_u64_u8(vshrn_n_u16(vreinterpretq_u16_u8(cmp_8bit), 4)), 0);
