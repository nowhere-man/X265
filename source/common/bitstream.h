...
public:
    ...
    void resetBits()
    {
        m_partialByteBits = m_byteOccupancy = 0;
        m_partialByte = 0;
    }
    ...
    void copyBits(Bitstream *stream)
    {
        m_partialByteBits = stream->m_partialByteBits;
        m_byteOccupancy = stream->m_byteOccupancy;
...
static inline int bs_size_ue(unsigned int val) { return bitSize[val + 1]; }
