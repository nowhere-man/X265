...
template <int tuSize> void intraFilter(const pixel* samples, pixel* filtered) /* 1:2:1 filtering of left and top reference samples */
{
...
template <int width> void intra_pred_dc_c(pixel* dst, intptr_t dstStride, const pixel* srcPix, int /*dirMode*/, int bFilter)
{
    int k, l;
...
template <int log2Size> void planar_pred_c(pixel* dst, intptr_t dstStride, const pixel* srcPix, int /*dirMode*/, int /*bFilter*/)
{
    ...
    for (int y = 0; y < blkSize; y++) {
        for (int x = 0; x < blkSize; x++) {
            dst[y * dstStride + x] =
                (pixel)(((blkSize - 1 - x) * left[y] + (blkSize - 1 - y) * above[x] + (x + 1) * topRight + (y + 1) * bottomLeft + blkSize) >>
                        (log2Size + 1));
        }
    }
}

template <int width> void intra_pred_ang_c(pixel* dst, intptr_t dstStride, const pixel* srcPix0, int dirMode, int bFilter)
{
    int width2 = width << 1;
...
template <int log2Size> void all_angs_pred_c(pixel* dest, pixel* refPix, pixel* filtPix, int bLuma)
{
