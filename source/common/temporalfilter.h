...
struct MotionEstimatorTLD {
    ...
    void motionEstimationLuma(MotionEstimatorTLD& m_tld, MV* mvs, uint32_t mvStride, pixel* src, int stride, int height, int width, pixel* buf,
                              int bs, int sRange, MV* previous = 0, uint32_t prevmvStride = 0, int factor = 1);
    ...
    int motionErrorLumaSSD(MotionEstimatorTLD& m_tld, pixel* src, int stride, pixel* buf, int x, int y, int dx, int dy, int bs,
                           int besterror = 8 * 8 * 1024 * 1024);

    int motionErrorLumaSAD(MotionEstimatorTLD& m_tld, pixel* src, int stride, pixel* buf, int x, int y, int dx, int dy, int bs,
                           int besterror = 8 * 8 * 1024 * 1024);

    ~MotionEstimatorTLD() { predPUYuv.destroy(); }
};
