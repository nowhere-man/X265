...
enum PredMode { MODE_NONE = 0, MODE_INTER = (1 << 0), MODE_INTRA = (1 << 1), MODE_SKIP = (1 << 2) | MODE_INTER };
...
public:
    ...
    {
        return (m_cbf[0][absPartIdx] || ((m_chromaFormat != X265_CSP_I400) && (m_cbf[1][absPartIdx] || m_cbf[2][absPartIdx])));
    }
    ...
    int getPMV(InterNeighbourMV* neighbours, uint32_t reference_list, uint32_t refIdx, MV* amvpCand, MV* pmv, uint32_t puIdx = 0,
               uint32_t absPartIdx = 0) const;
#else
    ...
    uint32_t getBestRefIdx(uint32_t subPartIdx) const
    {
        return ((m_interDir[subPartIdx] & 1) << m_refIdx[0][subPartIdx]) | (((m_interDir[subPartIdx] >> 1) & 1) << (m_refIdx[1][subPartIdx] + 16));
    }
    uint32_t getPUOffset(uint32_t puIdx, uint32_t absPartIdx) const
    {
        return (partAddrTable[(int)m_partSize[absPartIdx]][puIdx] << (m_slice->m_param->unitSizeDepth - m_cuDepth[absPartIdx]) * 2) >> 4;
    }
...
protected:
    template <typename T> void setAllPU(T* p, const T& val, int absPartIdx, int puIdx);
...
struct CUDataMemPool {
    ...
    {
        charMemBlock = NULL;
        trCoeffMemBlock = NULL;
        mvMemBlock = NULL;
        distortionMemBlock = NULL;
        dynRefineRdBlock = NULL;
        dynRefCntBlock = NULL;
        dynRefVarBlock = NULL;
    }
