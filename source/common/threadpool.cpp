...
void WorkerThread::threadMain()
{
    ...
    while (m_pool.m_isActive) {
        ...
        do {
            ...
            int curPriority = (m_curJobProvider->m_helpWanted) ? m_curJobProvider->m_sliceType : INVALID_SLICE_PRIORITY + 1;
            int nextProvider = -1;
            for (int i = 0; i < m_pool.m_numProviders; i++) {
                if (m_pool.m_jpTable[i]->m_helpWanted && m_pool.m_jpTable[i]->m_sliceType < curPriority) {
                    nextProvider = i;
...
ThreadPool* ThreadPool::allocThreadPools(x265_param* p, int& numPools, bool isThreadsReserved)
{
    ...
    if ((threadsPerPool[numNumaNodes] > MAX_POOL_THREADS) && ((threadsPerPool[numNumaNodes] % MAX_POOL_THREADS) < (MAX_POOL_THREADS / 2))) {
        threadsPerPool[numNumaNodes] -= (threadsPerPool[numNumaNodes] % MAX_POOL_THREADS);
        x265_log(p, X265_LOG_DEBUG,
                 "Creating only %d worker threads beyond specified numbers with --pools (if specified) to prevent asymmetry in pools; may not use "
                 "all HW contexts\n",
...
ThreadPool::ThreadPool() { memset(this, 0, sizeof(*this)); }
...
void ThreadPool::setCurrentThreadAffinity() { setThreadNodeAffinity(m_numaMask); }
