...
template <int lx, int ly> int sad(const pixel* pix1, intptr_t stride_pix1, const pixel* pix2, intptr_t stride_pix2)
{
    int sum = 0;
...
template <int lx, int ly> int sad(const int16_t* pix1, intptr_t stride_pix1, const int16_t* pix2, intptr_t stride_pix2)
{
    int sum = 0;
...
template <int lx, int ly> void sad_x3(const pixel* pix1, const pixel* pix2, const pixel* pix3, const pixel* pix4, intptr_t frefstride, int32_t* res)
{
    res[0] = 0;
    res[1] = 0;
    res[2] = 0;
...
template <int lx, int ly> int ads_x4(int encDC[4], uint32_t* sums, int delta, uint16_t* costMvX, int16_t* mvs, int width, int thresh)
{
    int nmv = 0;
    for (int16_t i = 0; i < width; i++, sums++) {
        int ads = abs(encDC[0] - long(sums[0])) + abs(encDC[1] - long(sums[lx >> 1])) + abs(encDC[2] - long(sums[delta])) +
                  abs(encDC[3] - long(sums[delta + (lx >> 1)])) + costMvX[i];
        if (ads < thresh) {
            mvs[nmv++] = i;
        }
    }
...
template <int lx, int ly> int ads_x2(int encDC[2], uint32_t* sums, int delta, uint16_t* costMvX, int16_t* mvs, int width, int thresh)
{
    int nmv = 0;
    for (int16_t i = 0; i < width; i++, sums++) {
        int ads = abs(encDC[0] - long(sums[0])) + abs(encDC[1] - long(sums[delta])) + costMvX[i];
        if (ads < thresh) {
            mvs[nmv++] = i;
        }
    }
...
template <int lx, int ly> int ads_x1(int encDC[1], uint32_t* sums, int, uint16_t* costMvX, int16_t* mvs, int width, int thresh)
{
    int nmv = 0;
    for (int16_t i = 0; i < width; i++, sums++) {
        int ads = abs(encDC[0] - long(sums[0])) + costMvX[i];
        if (ads < thresh) {
            mvs[nmv++] = i;
        }
    }
...
template <int lx, int ly, class T1, class T2> sse_t sse(const T1* pix1, intptr_t stride_pix1, const T2* pix2, intptr_t stride_pix2)
{
    sse_t sum = 0;
    int tmp;
...
int satd4(const pixel* pix1, intptr_t stride_pix1, const pixel* pix2, intptr_t stride_pix2)
{
    ...
    for (int row = 0; row < h; row += 4) {
        for (int col = 0; col < w; col += 4) {
            satd += satd_4x4(pix1 + row * stride_pix1 + col, stride_pix1, pix2 + row * stride_pix2 + col, stride_pix2);
        }
    }
...
{
    int sum = _sa8d_8x8(pix1, i_pix1, pix2, i_pix2) + _sa8d_8x8(pix1 + 8, i_pix1, pix2 + 8, i_pix2) +
...
template <int size> sse_t pixel_ssd_s_c(const int16_t* a, intptr_t dstride)
{
    sse_t sum = 0;
...
template <int size> void blockfill_s_c(int16_t* dst, intptr_t dstride, int16_t val)
{
    for (int y = 0; y < size; y++) {
        for (int x = 0; x < size; x++) {
            dst[y * dstride + x] = val;
...
template <int size> void cpy2Dto1D_shl(int16_t* dst, const int16_t* src, intptr_t srcStride, int shift)
{
...
template <int size> void cpy2Dto1D_shr(int16_t* dst, const int16_t* src, intptr_t srcStride, int shift)
{
    X265_CHECK(((intptr_t)dst & 15) == 0, "dst alignment error\n");
    X265_CHECK((((intptr_t)src | (srcStride * sizeof(*src))) & 15) == 0 || size == 4, "src alignment error\n");
    X265_CHECK(shift > 0, "invalid shift\n");
...
template <int size> void cpy1Dto2D_shl(int16_t* dst, const int16_t* src, intptr_t dstStride, int shift)
{
...
template <int size> void cpy1Dto2D_shr(int16_t* dst, const int16_t* src, intptr_t dstStride, int shift)
{
    X265_CHECK((((intptr_t)dst | (dstStride * sizeof(*dst))) & 15) == 0 || size == 4, "dst alignment error\n");
    X265_CHECK(((intptr_t)src & 15) == 0, "src alignment error\n");
    X265_CHECK(shift > 0, "invalid shift\n");
...
template <int blockSize> void getResidual(const pixel* fenc, const pixel* pred, int16_t* residual, intptr_t stride)
{
...
template <int blockSize> void transpose(pixel* dst, const pixel* src, intptr_t stride)
{
...
static void weight_sp_c(const int16_t* src, pixel* dst, intptr_t srcStride, intptr_t dstStride, int width, int height, int w0, int round, int shift,
                        int offset)
{
    int x, y;
...
static void frame_init_lowres_core(const pixel* src0, pixel* dst0, pixel* dsth, pixel* dstv, pixel* dstc, intptr_t src_stride, intptr_t dst_stride,
...
static void frame_subsample_luma(const pixel* src0, pixel* dst0, intptr_t src_stride, intptr_t dst_stride, int width, int height)
{
...
static float ssim_end_4(int sum0[5][4], int sum1[5][4], int width)
{
    ...
    for (int i = 0; i < width; i++) {
        ssim += ssim_end_1(sum0[i][0] + sum0[i + 1][0] + sum1[i][0] + sum1[i + 1][0], sum0[i][1] + sum0[i + 1][1] + sum1[i][1] + sum1[i + 1][1],
...
template <int size> uint64_t pixel_var(const pixel* pix, intptr_t i_stride)
{
...
template <int size> int psyCost_pp(const pixel* source, intptr_t sstride, const pixel* recon, intptr_t rstride)
{
    ...
    if (size) {
        ...
        for (int i = 0; i < dim; i += 8) {
            for (int j = 0; j < dim; j += 8) {
                ...
                int sourceEnergy =
                    sa8d_8x8(source + i * sstride + j, sstride, zeroBuf, 0) - (sad<8, 8>(source + i * sstride + j, sstride, zeroBuf, 0) >> 2);
                int reconEnergy =
                    sa8d_8x8(recon + i * rstride + j, rstride, zeroBuf, 0) - (sad<8, 8>(recon + i * rstride + j, rstride, zeroBuf, 0) >> 2);
...
template <int bx, int by> void blockcopy_pp_c(pixel* a, intptr_t stridea, const pixel* b, intptr_t strideb)
{
    for (int y = 0; y < by; y++) {
        for (int x = 0; x < bx; x++) {
            a[x] = b[x];
...
template <int bx, int by> void blockcopy_ss_c(int16_t* a, intptr_t stridea, const int16_t* b, intptr_t strideb)
{
    for (int y = 0; y < by; y++) {
        for (int x = 0; x < bx; x++) {
            a[x] = b[x];
...
template <int bx, int by> void blockcopy_sp_c(pixel* a, intptr_t stridea, const int16_t* b, intptr_t strideb)
{
    for (int y = 0; y < by; y++) {
        for (int x = 0; x < bx; x++) {
...
template <int bx, int by> void blockcopy_ps_c(int16_t* a, intptr_t stridea, const pixel* b, intptr_t strideb)
{
    for (int y = 0; y < by; y++) {
        for (int x = 0; x < bx; x++) {
            a[x] = (int16_t)b[x];
...
template <int bx, int by> void pixel_sub_ps_c(int16_t* a, intptr_t dstride, const pixel* b0, const pixel* b1, intptr_t sstride0, intptr_t sstride1)
{
    for (int y = 0; y < by; y++) {
        for (int x = 0; x < bx; x++) {
...
template <int bx, int by> void pixel_add_ps_c(pixel* a, intptr_t dstride, const pixel* b0, const int16_t* b1, intptr_t sstride0, intptr_t sstride1)
{
    for (int y = 0; y < by; y++) {
        for (int x = 0; x < bx; x++) {
...
static void planecopy_sp_shl_c(const uint16_t* src, intptr_t srcStride, pixel* dst, intptr_t dstStride, int width, int height, int shift,
                               uint16_t mask)
{
    for (int r = 0; r < height; r++) {
        for (int c = 0; c < width; c++) {
