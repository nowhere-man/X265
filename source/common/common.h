...
template <typename T> inline T x265_clip3(T minVal, T maxVal, T a) { return x265_min(x265_max(minVal, a), maxVal); }
...
static inline int8_t x265_signOf(int32_t x) { return (x >> 31) | ((int32_t)((((uint32_t)-x)) >> 31)); }
...
#define COEF_REMAIN_BIN_REDUCTION \
    3  // indicates the level at which the VLC
...
enum SaoMergeMode { SAO_MERGE_NONE, SAO_MERGE_LEFT, SAO_MERGE_UP };
...
enum SignificanceMapContextType { CONTEXT_TYPE_4x4 = 0, CONTEXT_TYPE_8x8 = 1, CONTEXT_TYPE_NxN = 2, CONTEXT_NUMBER_OF_TYPES = 3 };
