...
ScalerFilter::ScalerFilter() : m_filtLen(0), m_filtPos(NULL), m_filt(NULL), m_sourceSlice(NULL), m_destSlice(NULL) { }
...
int ScalerFilterManager::init(int algorithmFlags, VideoDesc *srcVideoDesc, VideoDesc *dstVideoDesc)
{
    ...
    m_ScalerFilters[1]->initCoeff(m_algorithmFlags, crXInc, m_crSrcW, m_crDstW, filterAlign, 1 << 14, getLocalPos(m_crSrcHSubSample, srcHCrPos),
                                  getLocalPos(m_crDstHSubSample, dstHCrPos));
    ...
    m_ScalerFilters[3]->initCoeff(m_algorithmFlags, crYInc, m_crSrcH, m_crDstH, filterAlign, 1 << 12, getLocalPos(m_crSrcVSubSample, srcVCrPos),
                                  getLocalPos(m_crDstVSubSample, dstVCrPos));
    ...
    for (int dstY = 0; dstY < dstH; dstY++) {
        ...
        if (firstLumSrcY > lastInLumBuf) {
            ...
            if (hasLumHoles) {
                ...
                hout_slice->m_plane[0].sliceHor = hout_slice->m_plane[3].sliceHor = 0;
            }
        ...
        if (firstCrSrcY > lastInCrBuf) {
            ...
            if (hasCrHoles) {
                ...
                hout_slice->m_plane[1].sliceHor = hout_slice->m_plane[2].sliceHor = 0;
            }
...
ScalerSlice::ScalerSlice() : m_width(0), m_hCrSubSample(0), m_vCrSubSample(0), m_isRing(0), m_destroyLines(0)
{
...
int ScalerSlice::initFromSrc(uint8_t *src[4], const int stride[4], int srcW, int lumY, int lumH, int crY, int crH, int relative)
{
    ...
    uint8_t *const src_[m_numSlicePlane] = {src[0] + (relative ? 0 : start[0]) * stride[0], src[1] + (relative ? 0 : start[1]) * stride[1],
                                            src[2] + (relative ? 0 : start[2]) * stride[2], src[3] + (relative ? 0 : start[3]) * stride[3]};
